...
- `.codex/skills/*/SKILL.md` 中反引号包裹的具体项目路径和 fenced code block 里的 `node ...mjs` 验证脚本必须真实存在，由 `node scripts/ci/check-ai-governance.mjs` 反查，避免 skill 迁移后留下不可执行引用。
