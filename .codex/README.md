...
本目录存放 Codex 的项目级 agent profiles、hooks 和说明；项目 skills 的唯一源码位于 `.agents/skills/`，由 Codex 在仓库上下文中直接发现。

仓库本身仍是 JSONUtils 项目，只有 `plugins/<name>/` 是可分发 plugin package。项目插件以 `.agents/plugins/marketplace.json`、`.agents/plugins/plugin-lock.json` 和 `plugins/` 为版本化 source of truth；该 marketplace 是项目内插件目录清单/索引文件，而非自动安装器，clone/open 只获得索引、content lock 和源码。先运行 `node scripts/ci/manage-project-plugins.mjs --check` 诊断状态（不执行 marketplace/plugin add/remove/enable/disable 或 lock 写入），明确同意后才用 `--apply` 注册/安装并在新任务加载；用户 marketplace/cache 只是派生安装状态，不能反向成为项目权威源。
...
- `agents/explorer.toml`、`agents/worker.toml`、`agents/verifier.toml`: 项目级 Codex specialist profiles，分别承担只读调查、白名单实现和只验证不修复。
- `hooks.json`、`hooks/session-start-governance.mjs`: 单一只读 `SessionStart` 治理 handoff，固定有界 context，不读取敏感 lifecycle 正文或扩大执行能力。

## 使用方式

当前项目以根目录 `AGENTS.md` 作为通用入口，并以 `.agents/skills/` 作为项目 skill 的唯一版本化 source。维护者 clone 后无需把它们复制到个人目录；打开仓库的新任务即可发现。不要再在 `.codex/skills/` 放同名副本，否则当前 Codex 会把两份 skill 同时注入。
...
- 项目 MCP 声明、当前任务注册发现和工具选择分开评分；项目插件 `plugins/jsonutils-governance-mcp@0.2.0` 安装/冒烟只算候选，新任务实际发现工具前记录 open signal 与 blocked 3×2 paired canary，不写 selection outcome。
- 项目插件 `plugins/codex-mcp-config-auditor@0.2.0` 只在用户本机显式运行时审计敏感静态 MCP header 名称；返回值固定不含配置值、hash、长度、preview、原始错误或环境值。源码与测试是项目资产，本机 finding 不是行为通过证据。
...
- 项目插件 0.4.0 的三 fake workload runtime probe 使用 `node scripts/ci/run-ai-evolution-cases.mjs --case codex-external-controller-runtime-probe-boundary --json` 锁定本机安装副本自报边界；审核镜像政策落地前禁用 runtime，`not-run` 或未来的 `passed-subset` 都不证明 runtime/controller/user namespace/signer 隔离，不产生 outcome。
...
- Codex 委派优先路由到 `.codex/agents/` 的固定 explorer、worker、verifier；explorer 必须只读，worker 必须有父任务写入白名单，verifier 的 `workspace-write` 只允许验证命令产生临时/忽略产物且不得修改源码、规则、配置、账本或版本。
- Codex project hook 只允许 `.codex/hooks.json` 的 `SessionStart` `startup|resume` advisory；项目与当前定义都需显式信任，禁止 bypass trust、其它事件、prompt/transcript/环境读取、网络、写入和阻断。配置与单测只算 component evidence，新任务真实触发前 behavior 为 unknown。
