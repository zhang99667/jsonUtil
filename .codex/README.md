...
- 项目源码、trusted project config 加载、marketplace 可发现、插件安装/启用、新任务实际注册与 runtime/signer 分开取证；`.codex/config.toml` 是 Codex 的项目原生入口，`plugins/jsonutils-governance-mcp@0.2.2` 只保留为兼容分发包。当前 upstream 源码表明同名 direct config 优先于 plugin server，但这仍只是 component inference；在 fresh task 实际观察前保持 behavior unknown。
- 项目插件 `plugins/codex-mcp-config-auditor@0.2.2` 只在用户本机显式运行时审计敏感静态 MCP header 名称；返回值固定不含配置值、hash、长度、preview、原始错误或环境值。源码与测试是项目资产，本机 finding 不是行为通过证据。
