...
8. 修改 `.codex/skills/*/SKILL.md` 时保留 frontmatter `name`/`description`/`version`/`tags`（`name` 必须等于 skill 目录名，`version` 使用 `x.y.z`，`tags` 使用非空数组）和四个核心章节，确保 skill 仍可发现、可迁移、可版本化、可验证。
