...
*   **结构导航**: 以树形结构浏览当前 PREVIEW JSON / JSON Lines，支持字段/路径多关键词与模糊搜索、搜索命中高亮、节点类型筛选、节点详情、字符串语义预览（URL、Scheme、JWT、Base64、资源 URL、邮箱、电话、日期、颜色）、对象数组表格预览、表格列筛选、复制 JSONPath / JSON Pointer / 节点值 / 节点子树 / 搜索结果 JSON / 搜索结果 Markdown / 搜索结果 CSV / 表格 JSON / 表格 CSV，并可联动 JSONPath 面板定位高亮；树模型在 Worker 中异步构建，降低大 JSON 阻塞编辑器的风险。
