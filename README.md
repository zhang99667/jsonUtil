...
`corpus:snapshot:check` 用于校验脱敏真实 response 的解析质量基线；`corpus:snapshot:diff` 用于对比两份质量快照的覆盖率、CMD/资源字段、必需项失败和 cmdHandler 对齐趋势；`perf:scheme` 会优先复制真实 `data.video` 条目构造 50KB / 250KB response，用于对核心解析耗时、覆盖率、CMD 字段和资源字段下限做本地/CI 预算检查。
