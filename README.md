...
*   **JSON Schema 校验**: 支持从当前 SOURCE JSON 生成初始 Schema、粘贴校验、错误路径定位、编辑器内标记，以及本地收藏的导入/导出共享。
*   **CMD/Scheme 深度解析**:
...
*   **文件管理**:
    ...
    *   自动保存功能，防止数据丢失。
    *   支持打开本地文件和另存为。
    *   支持导入 `.har` 抓包文件，自动提取 request/response body 为派生 JSON 标签，生成接口摘要和不含 query 的短标签，并在深度解析报告中展示和筛选接口上下文。
*   **高度可定制**:
    *   自定义快捷键，打造个人专属工作流。
