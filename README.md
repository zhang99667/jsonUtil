...
*   **JSON Schema 校验**: 支持从当前 SOURCE JSON 按严格/宽松策略生成初始 Schema，并自动推断常见 `format`；支持粘贴校验、复制当前 Schema、复制/应用示例 JSON、动态对象、唯一数组和常见字符串 `pattern` 示例、标准 `format` 约束、问题分布、修复建议、修复清单、错误路径定位、编辑器内标记，以及本地收藏的导入/导出共享。
*   **CMD/Scheme 深度解析**:
