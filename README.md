...
*   **智能建议**: 根据当前 SOURCE 自动推荐下一步动作；复杂 response 会提示排查工作流，一键切到嵌套解析并打开深度解析报告；独立 CMD/Scheme 会提示 Scheme 面板，复杂 JSON 会提示结构导航，普通 HTTP(S) URL 只提示 URL 解码，避免把所有链接误当业务 Scheme。
*   **多种转换模式**:
