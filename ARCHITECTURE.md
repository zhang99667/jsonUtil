...
│   │   ├── JsonComparePanel.tsx   # 通用 JSON 语义对比、忽略路径与报告复制面板
...
│   │   ├── jsonSemanticDiff.ts    # 通用 JSON / JSON Lines 语义 diff、忽略路径和 Markdown 报告
...
| 模块 | 说明 |
|------|------|
| `jsonSchemaInference.ts` | 根据当前 SOURCE JSON 推断初始 JSON Schema，支持严格/宽松必填策略、常见字符串 format 推断，并通过前段、尾段、分散点和稀疏字段代表行采样限制长数组推断成本 |
