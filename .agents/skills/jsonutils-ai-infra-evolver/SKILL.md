...
- 修改或排查 grader、ledger、outcome/receipt、checker、trace、runner 或 controller 的字段、状态或协议时读 `evals/ai-governance/README.md`。
...
2. 可独立并行时委派默认 explorer/worker/verifier；AI 基建只读审计可用 ai-infra-auditor，主线程锁边界并整合，worker 必须收到写入白名单。
3. 建立治理、预算、MCP stdio、eval corpus、behavior outcome coverage 与 mandatory-context 双基线；时效结论核对一手来源。
4. AI 基建的 source of truth 必须入库；workspace/index/HEAD 依次只证明未 ignore、下一提交候选、当前提交可 clone。仓库不是 plugin，只有 `plugins/<name>/` 是插件包；manifest 的 `mcpServers` 只是 companion 指针，companion 使用 direct map 或 `mcp_servers`，不得复用 project-level `mcpServers` 包装。`.agents/plugins/marketplace.json` 不自动安装，先 `--check`，明确同意后才 `--apply`。个人 cache 不证明 registration/runtime trust。
...
7. 真实 outcome 绑定 trial receipt；legacy v1 仅留历史，`schemaVersion 3` 锁 `chain.sequence`、`chain.previousHash`、`supersession.previousOutcomeId` 和 `feedbackDisposition`。fixed runner 不写账；三个项目 writer 均 preview-first，仅本地显式写入，禁止自动化或手改 JSONL。字段由当前受控证据/source/ledger 派生；未验信 trace 永不计分，paired v4 任一基础设施无效整批不计分且 baseline 不进 outcome。细则按任务读 `evals/ai-governance/README.md`。
...
- 项目 Agent 的通用执行角色固定 explorer、worker、verifier；AI 基建专项只读审计另允许跨 Codex/Claude/Copilot 的 ai-infra-auditor。客户端权限配置只是默认值，父任务覆盖不放宽职责；静态 adapter 只算 component evidence，真实选择、强隔离和零写入保持 unknown，零写入需比较完整 workspace manifest。
