...
## v1.8.1053 (2026-08-30) - 固化 Skill 契约收集器边界

### 🐛 Bug 修复

- **注册表不可变**: 将 10 个 Codex Skill 契约收集器冻结为不可变有序数组，阻止同进程 importer 通过 `pop()` 等变异静默删除治理检查

### 🏗️ 架构与基础设施

- **失败顺序锁定**: 直接测试同时固定收集器数量、冻结状态、变异拒绝，以及跨结构/引用 collector 的诊断顺序；既有参数适配、失败文案和聚合职责保持不变
- **预算校准**: 23 行单责注册表不做机械拆分，配合不可变性负例将预算从 25 校准至 30 行，保留 7 行真实演进余量
- **证据边界不变**: 本次只加固仓内 component 契约，不增加 behavior coverage，不解除 protected runtime、fresh-task registration 或 signer trust 的外部阻塞
...
- **JSONPath Tab 隔离**: 将线上 v1.8.1039 的 JSONPath 查询工作区按 Tab 隔离改动纳入 Git，包含查询状态隔离、异步归属保护和生命周期清理
- **SOURCE/PREVIEW 同步滚动**: 将线上 v1.8.1038 的双向比例同步滚动和回环防护纳入 Git

### 🚀 优化与改进

- **深度解析收敛**: 保留 transformations.ts 从 938 行降至 759 行的职责拆分，上下文收集器与 Scheme 步骤模块不变

### 🏗️ 架构与基础设施

- **路径原语单源**: isPathWithin 从 11 处重复实现收敛到共享模块
- **Maven 测试兼容**: CI 与文档统一使用 subclass mock maker

## v1.8.1039 (2026-08-26) - 单源路径包含原语

### 🚀 优化与改进

- **isPathWithin 单源**: 将 11 处重复的 `path.relative` 包含检查收敛到 `aiGovernancePathWithin.mjs`，统一 parent/child 语义并消除运算符优先级差异

### 🏗️ 架构与基础设施

- **直接负例**: 新增相同路径、内部路径、父路径和兄弟逃逸测试，预算规则同步登记新模块

## v1.8.1038 (2026-08-26) - 收敛深度解析核心职责

### 🚀 优化与改进

- **上下文收集单源**: 深度解析的待检查候选、运行时占位符和性能警告迁移到独立有界收集器，核心转换文件减少约 220 行
- **Scheme 步骤权威**: CMD/URL/Base64 JSON 展开步骤与参数分层脱敏摘要由纯函数模块统一生成，避免展示头与可逆性规则散落
- **重复解码消除**: Scheme 命中后复用同一次解码结果，不再为占位符二次解析同一字符串，并保留原有还原行为

### 🏗️ 架构与基础设施

- **收集器与步骤负例**: 新增预览截断、来源路径绑定、占位符容量、参数摘要截断、CMD JSON 展开与非对象结果拒绝回归，锁定新模块边界
- **后端测试兼容**: CI 与维护文档统一使用 subclass mock maker，避免禁止 JVM self-attach 的本机沙箱把 Mockito 初始化限制误报为业务失败
...
- **Tab 滚动与折叠隔离**: 改由 Monaco 按每个 Tab 的唯一模型路径同步保存和恢复滚动、光标与折叠状态，覆盖标签栏切换和结构导航直接新建 Tab 等全部入口，避免新 Tab 继承旧位置或返回后折叠节点自动展开
- **真实双长 Tab 回归**: 新增两个长 JSON Tab 与结构子树新建 Tab 的端到端用例，分别断言目标 Tab 保持顶部展开、原 Tab 恢复深处滚动和折叠状态

### 🏗️ 架构与基础设施

- **维护验收规则回写**: `jsonutils-maintainer` 升级到 `0.1.41`，要求 Monaco Tab 状态隔离使用双长内容并覆盖非标签栏切换入口；同步移除已删除手写恢复模块的可维护性预算引用

## v1.8.1035 (2026-08-21) - 结构子树一键复制到新标签

### ✨ 新特性

- **复制并新建 Tab**: 结构导航的对象和数组节点新增一键操作，准确复制格式化子树并在独立未保存 Tab 中打开，同时保留原 Tab 内容与视图状态

## v1.8.1034 (2026-08-21) - 稳定长数组项数提示验收

### 🏗️ 架构与基础设施

- **SOURCE 粘贴 E2E 就绪门禁**: 公共粘贴 helper 在设置剪贴板和点击前先等待 SOURCE Monaco 与草稿恢复完成，避免预构建页面加载较快时迟到初始化清空输入，并稳定覆盖长数组 `4 项` 到 `3 项` 的实时刷新

## v1.8.1033 (2026-08-21) - 独立对比工作台与编辑器状态隔离

### 🐛 Bug 修复

- **Tab 滚动隔离**: 统一由文件 Tab 保存和恢复 Monaco 视图状态，取消过期恢复帧，避免快速切换时滚动位置串到其他 Tab

### 🎨 UI/UE 优化

- **双栏 JSON 对比工作台**: 基准与对比内容改为独立可编辑 Monaco，两侧均支持粘贴、格式化和清空；输入区高度可拖动或用键盘调整，差异结果保留路径复制、定位与忽略规则
- **长数组项数提示**: 复用现有 JSON source map 单次扫描，在数组 `[` 后显示如 `4 项` 的低干扰标记，内容变化时自动刷新
...
- **CI descriptor 锁同步**: required-read 字节绑定变化后同步 trace policy 派生 descriptor SHA，并将全量 Node script tests 纳入发布前直接回归，修复 GitHub CI 唯一失败
...
- **部署回填竞态**: full 部署在前端入口完成静态目录初始化且健康检查通过后才回填旧 hash 资源，避免 `docker compose cp` 与容器启动阶段交叉写入时因目标文件瞬时变动而中断发布
- **顺序回归锁定**: 静态资源发布门禁锁定“健康检查 → 旧资源回填”顺序，覆盖回填被提前的负例，不改变已有旧资源保留时长和公网复查语义
- **证据边界不升级**: 源码冻结后仅通过 preview-first 受控 writer 刷新 3 条既有 deterministic case 的 source-state 时效，不扩大 behavior 分母、trace 信任或仓外 runtime/signer 声明
...
- **入口清单直连**: 入口、工具薄入口与 Claude README 规则直接绑定入口核心权威清单，移除 runtime 聚合模块对 4 个入口领域常量的兼容 re-export
- **依赖方向回归**: 新增模块身份断言，锁定 runtime 聚合模块不再代理入口领域导出；引用内容、顺序与失败语义保持不变
- **维护余量恢复**: runtime 引用模块从 42 行降至 35 行，不拆分职责单一的 29/30 入口核心清单，也不提高既有维护预算
- **证据边界不升级**: 重构不修改 case、corpus、behavior coverage 或仓外 runtime/signer blocker；源码冻结后仅由受控 writer 刷新既有 deterministic source-state 时效

## v1.8.1012 (2026-08-09) - 移除 AI 文档引用兼容 façade

### 🏗️ 架构与基础设施

- **引用清单直连**: docs/AI 引用规则分别直接绑定配置分层与工具索引权威清单，移除配置模块中无消费者的工具索引兼容 re-export
- **依赖方向回归**: 新增模块身份断言，锁定引用内容仍复用原始清单且配置模块不再代理工具索引；引用顺序、失败语义与行为评测边界不变
...
- **Calibration mutation 叶子**: 将 17 类 failure-taxonomy mutation 路由从合成 packet/result context 工厂下沉为独立纯投影，原 fixture API 保持同引用重导出
...
- **MCP 观察窗完整覆盖**: 两条真实 stdio 集成中的全部 worker-backed 工具调用统一使用 45 秒客户端观察窗，修复 freshness 等后续调用仍沿用 30 秒或默认 5 秒而在受载 CI 上超时的问题

## v1.8.1004 (2026-07-30) - 加固 trial receipt 账本原始读取边界

### 🏗️ 架构与基础设施

- **Receipt 稳定有界读取**: trial receipt ledger 只接受 8 MiB 内的稳定单链接普通文件，拒绝 symlink、hardlink、非法 UTF-8、BOM 与物理行/记录超限，读取失败不再回显本机路径
- **结构失败闭合**: malformed 或非紧凑 JSON 会清空可消费 receipt 索引并按物理记录计入 invalid，避免坏行被漏计后仍暴露局部有效证据
...
- **跨平台快捷键回归**: E2E 按应用默认配置发送 `Meta+S`，避免 Linux CI 将宿主平台映射成未配置的 `Control+S` 而漏触发手动保存

## v1.8.1001 (2026-07-30) - 拆分自动保存回归层级

### 🐛 Bug 修复

- **保存回归分层**: E2E 只验证快捷键将格式化后的 PREVIEW 写入文件，fake-timer 单测直接锁定取消自动保存先于文件写入，避免端到端测试通过共享延迟猜测内部定时器身份

## v1.8.1000 (2026-07-30) - 隔离自动保存定时器控制

### 🐛 Bug 修复

- **目标定时器隔离**: 自动保存 E2E 只接管启用开关后的首个 1000ms 防抖定时器，后续快捷键保存产生的 UI 定时器继续使用原生实现，避免把无关 timer 误判成未取消的自动保存

## v1.8.999 (2026-07-30) - 固定自动保存防抖回归

### 🐛 Bug 修复

- **PREVIEW 保存防抖验证**: E2E 直接控制并断言待取消的自动保存定时器，避免用真实 1 秒防抖窗口与 CI 调度速度竞跑，同时不冻结快捷键保存依赖的全局异步链路

## v1.8.998 (2026-07-30) - 固定自动保存回归前置状态

### 🐛 Bug 修复

- **PREVIEW 手动保存时序**: 先在关闭自动保存时等待 PREVIEW 就绪，再开启自动保存并确认进入等待状态后立即手动保存，避免全局 fake clock 冻结快捷键异步链路，同时校验手动写入完成后不存在迟到的 SOURCE 写入

## v1.8.997 (2026-07-30) - 固定自动保存竞态测试时钟

### 🐛 Bug 修复

- **PREVIEW 手动保存回归**: 在自动保存防抖窗口内暂停并推进浏览器时钟，确保慢速 CI runner 验证的是待触发 SOURCE 自动保存被取消，而不是运行速度造成的过期前置条件

## v1.8.996 (2026-07-30) - 加固 CI 慢速环境 E2E 时序

### 🐛 Bug 修复

- **慢速 runner 编辑器交互**: 保存 PREVIEW 前等待焦点状态切换完成，并在 Scheme 弹窗关闭后等待图标可操作再点击，避免 GitHub runner 上的焦点与弹窗竞态

## v1.8.995 (2026-07-30) - 修复设置备份与 E2E 回归

### 🐛 Bug 修复

- **原生弹窗内配置导出**: 设置弹窗打开时将临时下载链接挂载到当前对话框，避免浏览器 inert 规则拦截首次配置备份下载
- **前端 E2E 稳定性**: 统一关闭非引导用例中的 onboarding，更新中文化输出与通用设置默认页签断言，并等待 toast、PREVIEW 与剪贴板异步状态稳定后再校验

## v1.8.994 (2026-07-30) - 加固深层模板序列化
...
- **本机豁免声明常驻**: 本机私有配置即使尚未创建，也继续由资产账本声明豁免边界；真实文件存在时仍保持 Git index/HEAD 缺席和正文不读取

- **Scheme 扫描候选归一**: 对象强制定位和字符串 Scheme 共用限额、范围与结果装配分支，移除重复判断并恢复扫描器维护余量与直接测试预算合规

- **前端依赖与错误链加固**: 升级 Axios、PostCSS 和关联传递依赖，迁移到无已知高危链路的 ESLint React 配置，清零依赖审计并移除无效赋值、保留捕获异常的 `cause`

## v1.8.992 (2026-07-26) - 统一来源标签视觉

### 🎨 UI/UE 优化

- **内外层标签统一**: PREVIEW 外层与 Scheme 详情面板共用同一来源标签样式，协议头按 JSON 字符串形式紧随标签展示，不再出现整条 URL 横框

## v1.8.991 (2026-07-26) - 协议头字段标签化

### 🎨 UI/UE 优化

- **原字段位置替换**: Scheme 解析结果中，`__scheme__` / `__url__` 在原字段位置直接显示为 `Scheme 来源` / `URL 来源` 标签，协议头值继续完整展示
- **嵌套来源清晰化**: 根 Scheme 与嵌套 URL 按实际类型分别标注，不再让内部协议头字段名干扰阅读

## v1.8.990 (2026-07-26) - 来源标签回归字段位置

### 🎨 UI/UE 优化

- **独立来源信息行**: `Scheme 来源` / `URL 来源` 标签回到原协议头字段所在的视觉行，独立于 JSON 行号与业务内容展示，并保留解析前协议头
- **窄屏标签布局**: 来源类型固定可见，仅对过长协议头做省略；点击标签或左侧小眼睛仍可查看完整原始 Scheme / URL

## v1.8.989 (2026-07-26) - 来源标签补全协议头

### 🐛 Bug 修复

- **标签保留协议头**: `Scheme 来源` / `URL 来源` 标签追加自动解析前的原协议头，替代原有 `__scheme__` / `__url__` 展示字段；窄窗口自动省略且仍可悬浮或点击查看完整地址

## v1.8.988 (2026-07-26) - 协议来源改为编辑器标签

### 🎨 UI/UE 优化

- **Scheme 与 URL 来源标签**: 自动解析产生的协议头改为对象旁的 `Scheme 来源` / `URL 来源` 编辑器标签，不再显示成 `__scheme__` / `__url__` JSON 属性；标签与左侧小眼睛均可查看完整原始地址

### 🐛 Bug 修复

- **纯业务 JSON 与可逆回写**: PREVIEW 展示、复制和应用只包含业务 JSON，同名业务字段保持原样；协议来源继续由解析上下文安全参与嵌套反向编码

## v1.8.987 (2026-07-26) - 通用设置即时生效

### 🎨 UI/UE 优化

- **开关即时保存**: 通用设置开关点击后立即生效并进入现有持久化流程，移除取消与保存按钮，仅保留用于关闭面板的完成按钮

## v1.8.986 (2026-07-26) - 调整设置优先级

### 🎨 UI/UE 优化

- **设置页顺序**: 设置面板改为默认打开通用设置，并按通用设置、AI 配置、快捷键排列页签，让高频选项优先可见

## v1.8.985 (2026-07-26) - 查看完整原始地址

### 🐛 Bug 修复

- **来源头小眼睛**: 点击 `__url__` 或 `__scheme__` 左侧小眼睛时展开解析前的完整原始地址，查看模式禁止将完整地址误写回协议头字段

## v1.8.984 (2026-07-26) - 收敛来源头提示

### 🐛 Bug 修复

- **说明层防遮挡**: 来源头字段不再弹出大段说明，仅在左侧小眼睛保留一行完整地址查看提示，避免悬浮层覆盖相邻代码

## v1.8.983 (2026-07-26) - 精简来源头标记

### 🎨 UI/UE 优化

- **来源头去重**: 移除 `__url__` 与 `__scheme__` 字段旁重复的来源徽标，保留字段名和小眼睛入口，减少解析结果中的视觉噪音

## v1.8.982 (2026-07-26) - 全量未发布改动集成

### 🏗️ 架构与基础设施

- **并行改动统一集成**: 以最新主分支为基线合并本地未发布的前后端、测试和维护预算改动，统一使用未占用的 `1.8.982` 发布版本

### 🐛 Bug 修复

- **大文本与来源头保护**: PREVIEW 异步占位不再回写 SOURCE，深层 URL 与 Scheme 保留可编辑来源头并显示不进入 JSON 的解释标记
- **文件和请求生命周期**: 加固文件打开、保存、上传、预览、列表请求和浏览器副作用的取消、并发、卸载与异常边界
- **后端边界加固**: 收紧认证、管理员初始化、流量记录、统计投影、文件存储和 API 响应的数据与时钟边界；访问日志按实体字段上限安全截断，且只旁路持久化异常

### 🚀 优化与改进

- **Scheme 与 CMD 深层处理**: 拆分深层遍历、元数据、来源形态、结构化查询和回写职责，深层值改用栈安全序列化并补齐回归与性能预算
- **JSON 契约标准化**: 统一 JSON 解析、校验、持久化、栈安全序列化、Schema 示例、JSONPath、语义对比和 TypeScript 类型推断的值域边界
- **AI 修复稳健化**: 使用本地成熟修复器并加固提供方请求、响应归一化、超时、状态错误与敏感错误展示
- **管理后台与交互收敛**: 完善流量、文件和用户管理模型，统一模态框、浮动面板、标签页、引导、快捷键、剪贴板和错误提示生命周期

## v1.8.928 (2026-07-25) - 标注解析来源头

### 🎨 UI/UE 优化

- **URL 与 Scheme 来源标记**: PREVIEW 为工具实际注入的 `__url__` 和 `__scheme__` 来源头显示不进入 JSON 的专属标签与悬浮说明，帮助识别字段用途并保持复制和回写内容不变

## v1.8.927 (2026-07-25) - 补全深层 Scheme 协议头

### 🐛 Bug 修复

- **嵌套 Scheme 来源前缀**: 开启自动解析后，JSON 深层业务 Scheme 与 HTTP(S) 地址分别展示 `__scheme__` 和 `__url__` 协议头，并保持编辑后的逆向回写能力

## v1.8.926 (2026-07-25) - 阻断大文本占位回写

### 🐛 Bug 修复

- **PREVIEW 异步占位保护**: 大文本异步解析期间的内部处理中文案不再进入 PREVIEW 草稿同步链，避免占位文本覆盖 SOURCE 原文

## v1.8.925 (2026-07-20) - 补全嵌套解析来源前缀

### 🐛 Bug 修复

- **URL 与 Scheme 来源头**: 根 HTTP(S) 嵌套解析显示 `__url__`，自定义协议继续显示 `__scheme__`；字段冲突时安全回退，并在逆变换时只移除实际注入的展示字段

## v1.8.924 (2026-07-20) - 恢复 hash 路由嵌套解析

### 🐛 Bug 修复

- **结构化 data 参数**: 嵌套解析可识别 HTTP(S) hash 路由中以 `_data` 或 `Data` 结尾的结构化参数，并保持普通文本参数不被误展开

## v1.8.923 (2026-07-20) - 拆分 JSON Schema 推断模型

### 🚀 优化与改进

- **推断职责拆分**: 将格式识别、数组采样、递归推断与结构合并集中到独立模型模块，入口仅保留输入解析、可信摘要和结果序列化
