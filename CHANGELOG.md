...
- **规则回写追踪字段治理化**: 规则/skill 沉淀新增决策记录、回写追踪和锁定测试要求，Playbook、入口文档、Cursor/Comate 薄入口与项目 skill 同步写入，并由 AI 进化引用组和负向测试锁定，避免后续只写规则文本却无法追溯决策、同步范围和防退化测试
...
- **Cursor/Comate 入口纳入治理网**: `.cursorrules` 和 `.comate/rules/code-style.md` 收敛为薄入口，转发到主规范、Playbook 与治理命令，避免非 Codex/Claude 入口继续复制旧规则后漏掉版本闭环、子 Agent 委派和 AI 安全边界
