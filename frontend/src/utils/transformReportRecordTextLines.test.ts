...
describe('transformReportRecordTextLines', () => {
  ...
  it('输出记录明细并保留截断提示', () => {
    const lines = appendLines([
      {
        path: '$.cmd',
        labels: ['Scheme', 'CMD'],
        decodedPreview: 'sampleapp://v1/demo',
        ...
        commandSchemaRows: [
          { path: '$.cmd.schema', schema: 'sampleapp://v1/demo', source: 'cmd=1' },
          { path: '$.cmd.panel', schema: 'sampleapp://v1/panel', source: 'cmd=2' },
        ],
        nestedCommandFields: [{ path: '$.cmd.panel', preview: 'sampleapp://panel' }],
    ...
    expect(lines).toContain('  解析结果: sampleapp://v1/demo');
    ...
    expect(lines).toContain('  CMD Schema路径: $.cmd.schema=sampleapp://v1/demo');
    expect(lines).toContain('  CMD Schema路径: 还有更多未展示（总计 2 条）');
    expect(lines).toContain('  内部CMD字段: $.cmd.panel=sampleapp://panel');
