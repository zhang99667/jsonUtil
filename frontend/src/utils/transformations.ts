...
import { normalizeJsonEscapedSlashes } from './schemeEscapedPayloads.ts';
...
import { parseJsonValue, tryParseJsonValue } from './jsonValueGuards.ts';
import { stringifyJsonValue } from './jsonValueStringify.ts';
import {
  parseJsonCandidate,
  parseJsonInput,
  parseWrappedJsonInput,
  wrapJsonContent,
} from './jsonInputParser.ts';
import {
  expandJsonTree,
...
export { parseJsonInput };
export type { ParsedJsonInput } from './jsonInputParser.ts';
...
const isJsonScalarInput = (value: string): boolean => {
  if (!/^(?:"|-|\d|true$|false$|null$)/.test(value)) return false;
...
export const detectLanguage = (input: string): string => {
  ...
  if (isJsonScalarInput(trimmed)) return 'json';
...
const parseJsonString = (source: string): string => {
  const parsed = parseJsonValue(source);
  if (typeof parsed !== 'string') throw new TypeError('JSON 值不是字符串');
  return parsed;
};
...
export function deepParseWithContext(
  ...
  const processStringValue = (
    ...
    while (iterDepth < maxDepth) {
      ...
      try {
        ...
        if ((trimmed.startsWith('{') && trimmed.endsWith('}')) ||
          ...
          const jsonParsed = parseJsonValue(current);
    ...
    return { value: steps.length > 0 ? current : value };
  };

  const output = expandJsonTree(parsed, {
    transformString: (value, node) => processStringValue(
      value,
      node.path,
      node.depth,
      node.sourceLabel
    ),
    getArrayChildSourceLabel: getInheritedHarSourceLabel,
    getObjectChildSourceLabel: getNestedSourceLabelForField,
  });

  return {
    output: stringifyJsonValue(output, 2),
    context,
  };
}
...
export function inverseWithContext(
  ...
  try {
    const editedParsed = parseJsonValue(editedOutput);

    const result = restoreJsonWithRecords(editedParsed, {
      records: context.records,
      areValuesEqual: areJsonValuesSemanticallyEqual,
      applyStep: applyInverseStep,
    });
    ...
    const restoredJson = indentation === 0
      ...
      : stringifyJsonValue(result, indentation);
...
const encodeSchemeStringValue = (
  ...
  const schemeBefore = step.originalSchemeEscapedSlash
    ...
    : step.originalScheme;
  const hasDisplayHeaders = Boolean(step.schemeDisplayHeaders?.length);
  const schemeEncoding = layerType === 'url' && !hasDisplayHeaders
...
const sortJsonInput = (input: string): string => stringifyJsonOrLinesInput(
  input,
  value => stringifyJsonValue(sortJsonKeys(value), 2),
