...
describe('schemeQueryDetection', () => {
  ...
  it('提取带日志前缀的可解析 query 串', () => {
    expect(getSchemePrefixedQueryString('I/NadRender: cmd=%7B%22a%22%3A1%7D', options)).toEqual({
      prefix: 'I/NadRender: ',
      queryString: 'cmd=%7B%22a%22%3A1%7D',
    });
    expect(isDecodableSchemePrefixedQueryString('I/NadRender: cmd=%7B%22a%22%3A1%7D', options)).toBe(true);
  });

  it('普通 query 和不可解析前缀 query 不会命中前缀提取', () => {
    expect(getSchemePrefixedQueryString('cmd=%7B%22a%22%3A1%7D', options)).toBeNull();
    expect(getSchemePrefixedQueryString('I/NadRender: title=plain', options)).toBeNull();
  });
});
