...
type SyncInput = Parameters<typeof executeAppPreviewOutputSync>[0];
const runPreviewOutputSync = (overrides: Partial<SyncInput> = {}) => (
  executeAppPreviewOutputSync({
    previewText: '{"a":2}',
...
describe('appPreviewOutputSyncRunner', () => {
  it('格式化类 PREVIEW 校验失败时返回 invalid 结果', async () => {
    ...
    await expect(runPreviewOutputSync({
      previewText: '{bad',
      validateJsonMaybeAsync,
    })).resolves.toEqual({ status: 'invalid', validation: invalidResult });
  });
  ...
  it('格式化类 PREVIEW 校验异常时返回 failed 结果', async () => {
    ...
    await expect(runPreviewOutputSync({
      validateJsonMaybeAsync,
    })).resolves.toEqual({ status: 'failed', validation: PREVIEW_OUTPUT_SYNC_FAILED });
  });
  ...
  it('非格式化类 PREVIEW 直接反向转换且不触发 JSON 校验', async () => {
    ...
    expect(result).toEqual({ status: 'synced', nextSource: 'plain text' });
  });
});
