...
import {
  createDeferred,
  createFileSystemTestTab,
  createOpenedTextFile,
} from './fileSystemTestFixtures';
import {
...
describe('useFileSystem 并发标签更新', () => {
  ...
  it('异步打开与同批新建都保留', async () => {
    ...
    await scenario.fileSystem.openDroppedFiles([
      createOpenedTextFile('opened.json', '{"opened":true}'),
    ]);
  ...
  it('异步打开与同批关闭不丢文件或覆盖新活动标签', async () => {
    const scenario = useQueuedFileSystemScenario([
      createFileSystemTestTab('file-1', '{"first":true}'),
      createFileSystemTestTab('file-2', '{"second":true}'),
    ], 'file-1');
  ...
  it('关闭最后标签后才完成的慢打开会激活新文件', async () => {
    ...
    const openPromise = scenario.fileSystem.openDroppedFiles([
      createOpenedTextFile('opened.json', deferred.promise),
    ]);
    scenario.fileSystem.closeFile('file-1');
  ...
  it('同批连续关闭不会选中已排队关闭的标签', () => {
    const scenario = useQueuedFileSystemScenario([
      createFileSystemTestTab('file-1', '{"first":true}'),
      createFileSystemTestTab('file-2', '{"second":true}'),
      createFileSystemTestTab('file-3', '{"third":true}'),
    ], 'file-2');
  ...
  it.each([
    ...
    const scenario = useQueuedFileSystemScenario([
      { ...createFileSystemTestTab('file-1', '{"source":1}'), handle: oldHandle },
    ], 'file-1');
