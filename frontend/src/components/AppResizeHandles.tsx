...
export const AppSidebarResizeHandle: React.FC<AppSidebarResizeHandleProps> = ({
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={`absolute top-0 bottom-0 w-1 hover:bg-brand-primary focus:bg-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/70 cursor-col-resize z-20 transition-colors delay-100 ${isResizing ? 'bg-brand-primary' : 'bg-transparent'}`}
...
export const AppPaneResizeHandle: React.FC<AppPaneResizeHandleProps> = ({
  ...
  <div
    ...
    tabIndex={0}
    className={`w-1 hover:bg-brand-primary focus:bg-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/70 cursor-col-resize z-20 flex-shrink-0 transition-colors delay-100 ${isResizing ? 'bg-brand-primary' : 'bg-editor-sidebar'}`}
    onMouseDown={onMouseDown}
    onKeyDown={onKeyDown}
