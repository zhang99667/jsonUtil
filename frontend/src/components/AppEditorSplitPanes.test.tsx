...
describe('AppEditorSplitPanes', () => {
  it('装配 SOURCE/PREVIEW 分栏和中间 resize handle', () => {
    ...
    const root = assertElementLike(tree, 'AppEditorSplitPanes 应返回 React 元素');
    expect(root.props.className).toBe('flex-1 flex min-h-0');

    const children = root.props.children;
    expect(Array.isArray(children)).toBe(true);
    if (!Array.isArray(children)) throw new Error('AppEditorSplitPanes 应渲染三段子节点');
    expect(children).toHaveLength(3);
    expect(assertElementLike(children[0]).props['data-testid']).toBe('source-pane');
    expect(assertElementLike(children[2]).props['data-testid']).toBe('preview-pane');

    const handles = findByType(root, AppPaneResizeHandle);
