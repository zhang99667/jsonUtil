...
- 项目级 MCP 配置必须保持可审计：配置文件使用合法 JSON，包含 `mcpServers` 或 `servers`，不通过 shell 包装命令、绝对路径、上跳路径或缺失本地脚本隐藏执行边界，敏感字段以及 URL、args、header 字符串里的 token、password、api key 和 authorization 值只能使用环境变量引用。
