...
| Claude Code / Ducc | `CLAUDE.md`、`.claude/skills/jsonutils-maintainer/SKILL.md`、`.claude/skills/jsonutils-ai-infra-evolver/SKILL.md`、`.claude/agents/ai-infra-auditor.md` | `CLAUDE.md` 与 `AGENTS.md` 保持同源；skill adapter 固定路由 canonical，Agent adapter 提供只读 AI 基建审计 |
...
- `.claude/`、`.codex/`、`.cursor/rules/**/*.mdc`、MCP 配置（`.codex/config.toml`、`.mcp.json`、`.cursor/mcp.json`、`.vscode/mcp.json`）、`.comate/`、`docs/AI-*.md` 和 `rules/ai-*.md` 新增协作资产必须进入 `docs/AI-ASSET-REGISTRY.md`、治理清单、引用规则或显式豁免。
