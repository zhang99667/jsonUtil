...
- **CI**: 每次推送或 PR 运行前端类型检查、单元测试、解析质量与性能门禁、生产构建，后端 Maven 测试/打包，以及 Docker 镜像构建校验。
...
- `frontend`: `npm ci`、`npm run typecheck`、`npm test`、`npm run corpus:scheme`、`npm run corpus:snapshot:check`、`npm run perf:scheme -- --iterations 3 --strict`、`npm run build`、`npm run check:preloads`、`npm run test:e2e`
  ...
  - `npm run perf:scheme -- --iterations 3 --strict` 会通过复制真实 `data.video` 条目构造 50KB / 250KB 脱敏 response，校验核心解析耗时、覆盖率、CMD 结构、CMD 字段、资源字段、待检查和跳过数量，并上传 `scheme-performance-budget` artifact
