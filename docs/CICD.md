...
- **CI**: 每次推送或 PR 运行前端类型检查、Lint、单元测试、解析质量与性能门禁、生产构建，后端 Maven 测试/打包，以及 Docker 镜像构建校验。
...
- `frontend`: `npm ci`、`npm run typecheck`、`npm run lint`、`npm test`、`npm run corpus:scheme`、`npm run corpus:snapshot:check`、`npm run corpus:snapshot:diff -- --strict`、`npm run perf:scheme -- --iterations 3 --strict`、`npm run perf:jsonpath -- --iterations 3 --strict`、`npm run build`、`npm run check:preloads`、`npm run test:e2e`
