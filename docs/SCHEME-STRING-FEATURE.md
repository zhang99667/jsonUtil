...
- [ ] 新增 `schemeUtils.ts` 工具模块
- [ ] 实现 URL 编码/解码
- [ ] 实现 Base64 编码/解码
- [ ] 实现 JWT 解析（只读）
- [ ] 新增 `TransformMode.SCHEME_DECODE` 转换模式

**预计文件改动**：
```
frontend/src/utils/schemeUtils.ts      # 新增
frontend/src/utils/transformations.ts  # 修改
frontend/src/types.ts                  # 修改
frontend/src/components/ActionPanel.tsx # 修改
```
