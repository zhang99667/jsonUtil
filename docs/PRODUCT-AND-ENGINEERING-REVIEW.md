...
- 结构导航已补对象数组表格预览和全列搜索，选中列表型 response 数组时可直接看前几行字段分布，按列名从采样行完整字段中聚焦关键字段，并复制当前可见列的表格 JSON 或 CSV 继续排查。
...
- 结构导航已补字符串节点语义预览，选中 URL、Scheme、JWT、Base64、资源 URL、邮箱、电话、日期或颜色字符串时在节点详情区展示轻量标签；电话识别结合字段上下文，资源类型优先依据 URL 路径后缀，JWT/Base64 只展示结构摘要或长度，普通 HTTPS 与业务 Scheme 的入口判断保持不变。
...
- 数组表格视图: 已完成前 N 行对象数组表格、全列搜索和 CSV/JSON 复制；后续补展开嵌套对象和更多 CSV 选项，降低接口列表型 response 的阅读成本。
