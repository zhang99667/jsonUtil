...
- 当前仓库版本: v1.8.234。
- 当前测试规模: Vitest 全量 53 个测试文件、870 个用例，主应用 smoke E2E 96 个用例，前端源码文件约 123 个。
- 关键模块体量:
...
- quicktype: 多语言模型和运行时校验生成证明“从样本到代码”是高频链路；本项目已补 JSON 转 TS、结构节点 TS 类型复制和 Schema 示例，后续可把 Schema 与 TS 生成的字段覆盖率、可选字段和混合类型提示统一起来。
...
- CyberChef: 481 个 operation 和 recipe 模式说明“可组合流水线”比单个按钮更有生命力；本项目可先围绕 URL Decode -> JSON Repair -> Deep Parse -> JSONPath/Schema 形成可保存、可复制的轻量 recipe，而不是一次性复制 CyberChef 的大而全。
- jq / JSONPath Plus / JSON Path Finder: 查询工具的核心价值是低门槛定位字段、复制路径、回到结构上下文和组合过滤；本项目已先落地 JSONPath 字段名快捷查询和结果定位结构导航，后续可以补路径点击生成查询、查询历史分组、jq/JMESPath 只读预览。
...
- 查询降门槛: JSONPath 字段名直查与结果回到结构导航已落地，下一步可在结构导航节点上提供“复制递归字段查询”“复制精确路径查询”和“一键发送到 JSONPath 面板”，让不熟 JSONPath 的用户也能完成排查。
