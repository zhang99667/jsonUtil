...
- 最新真实响应探针结论: 可识别主入口 `samplevendor://vendor/ad/rewardImpl`，并固定自定义 Scheme、结构化 HTTPS、资源 URL、运行时占位符和 cmdHandler 预期子集，证明当前解析链路已能覆盖典型广告响应的多层结构。
...
- 已新增第二类脱敏落地页响应语料，覆盖 browser、deeplink、openapp、结构化 HTTPS 落地页、监测 URL、占位符和 cmdHandler 预期子集，质量快照样本数从 1 扩展到 2。
