...
本文档从 PM 和工程负责人两个视角评审 JSONUtils 当前状态，沉淀后续升级路线。结论基于当前仓库代码、文档、测试脚本、线上部署状态和最近围绕真实广告响应与 cmdHandler 对齐完成的改造。

## 一句话结论

JSONUtils 已经不只是通用 JSON 格式化工具，而是逐步形成了“复杂 JSON、广告 CMD 与真实响应解析工作台”。下一阶段最值得投入的方向不是继续堆单点按钮，而是把解析质量、样本沉淀、对比验证和协作交付做成闭环。
...
- 广告/客户端研发: 需要解析多层 URL、CMD、Scheme、Base64、运行时占位符和真实响应。
- 运营与管理后台用户: 需要用户、文件、访问统计和部署后的运行观测。

### 当前优势

- 主应用已经覆盖 JSON 编辑、转换、JSONPath、AI 修复、文件草稿、自动保存和多标签。
- Scheme/CMD 能力已经有明显专业化优势，支持整段响应、cmdHandler 风格复制、页面内对比、Top CMD Schema、占位符、Base64 后缀和质量快照。
...
- README 和架构文档偏通用 JSON 工具视角，对已经形成的 CMD/真实响应专业能力表达不足。
...
- 最新真实响应探针结论: 可识别主入口 `nadcorevendor://vendor/ad/rewardImpl`，并固定自定义 Scheme、结构化 HTTPS、资源 URL、运行时占位符和 cmdHandler 预期子集，证明当前解析链路已能覆盖典型广告响应的多层结构。
...
- 页面复制产物内置下一步命令: CMD 对比包、问题样本 JSON 和归档包会提示 `cmd:diff`、`samples:to-regression` 与语料基线校验命令，减少从页面排查切到 CLI 验证时的记忆成本。

建议功能:

- 样本库管理: 将真实响应的脱敏样本、问题样本、质量快照统一保存到 `fixtures` 或后端样本库。
- 质量基线: 每个样本记录覆盖率、CMD Schema 数量、占位符数量、待检查数量、跳过数量。
- 发布门禁: CI 跑固定样本集，阻止覆盖率下降、Schema 消失或待检查数量异常上升。
- cmdHandler 对齐集: 对核心样本保存内部 cmdHandler 预期结果，自动跑差异报告。

验收标准:

- 新增样本后可以一条命令生成质量快照。
- CI 能报告每个样本的解析质量变化。
- 对真实响应的主入口 Schema、关键嵌套 Schema 和占位符路径有固定断言。
...
- 一键生成排查报告: 聚合 Top CMD Schema、运行时占位符、未展开原因、cmdHandler 差异和建议动作。
- 路径定位组合包: 每条风险路径同时提供 JSONPath、原始来源路径、cmdHandler 实际结果片段。
- 占位符回填工作流增强: 回填模板应用后自动重新生成质量快照，展示替换前后差异。

验收标准:

- 用户粘贴整段响应后，3 次点击内可以复制一份可发给协作者的排查报告。
...
- HAR 请求提取: 打开 `.har` 文件时会生成不绑定原抓包文件句柄的派生 JSON 标签，提取请求/响应正文，补充方法/状态/主机/MIME 摘要、4xx/5xx/解析失败/截断等异常摘要和不含查询参数的短标签，并在深度解析报告里把 CMD/资源记录关联到接口上下文。

建议功能:

- YAML / XML / HAR 局部解析: 已完成 HAR 正文导入转换，后续可继续补 YAML / XML 到 JSON 视图，不急于做全功能编辑器。
- JSON Schema 校验: 支持从当前 JSON 按严格/宽松策略生成初始 Schema、粘贴 Schema 校验当前 JSON，并在编辑器中标记错误路径。
- HAR 请求提取增强: 已支持在深度解析报告中按接口域名、状态码和路径筛选 CMD/资源记录，并在导入摘要中提示需关注接口；后续可补独立的 HAR 载荷列表筛选和选中片段导出。
...
- AI 修复策略选择: 严格 JSON、保留注释、JSON Lines、真实响应保守修复。
...
- 性能观测: 上报大响应解析耗时区间和取消率，指导 Worker 与预算优化。

验收标准:

- 后台能回答“用户最常用什么功能”“失败主要在哪”“大响应是否卡顿”三个问题。
...
- 浏览器扩展版: 对网页响应、复制内容和网络载荷做快速格式化/深度解析。
...
- 单测可以读取语料，验证主入口 Schema、Top Schema、占位符和 Base64 后缀。
- 语料文件必须脱敏，不包含真实令牌、设备 ID、Cookie 或用户标识。
...
- README 应突出“复杂响应、CMD 与 Scheme 工作台”能力，否则外部用户只会把它理解成普通格式化工具。
...
- 已建立脱敏真实响应语料，并在 CI 中固定主 CMD Schema、热点 Schema、资源字段、占位符、扫描位置和 cmdHandler 预期子集。
- 已新增 `corpus:snapshot` / `corpus:snapshot:check`，支持一条命令生成质量快照，并在预期阈值不通过时阻断 CI。
- 新增语料样本如果缺少预期快照，会被质量门禁识别并失败，避免无基线样本混入。
- 质量快照会同步输出 cmdHandler 预期子集对齐结果；严格模式会拦截预期结果缺失、关键路径缺失和字段值不一致。
- 深度解析报告已支持一键复制协作排查报告，把诊断摘要、质量快照要点和当前页面内 cmdHandler 差异合并为可转发文本。
- 占位符回填模板应用后会展示并支持复制质量前后对比，能直接看到 CMD 结构、内部字段、占位符和风险计数变化。
- 深度解析报告已支持复制排查归档包，把诊断摘要、质量快照、脱敏问题样本、占位符回填模板和语料文件清单打包为安全沉淀材料。
- CI 会生成 Markdown 质量摘要并上传 JSON 快照构建产物，PM 和研发可以直接下载对照样本质量变化。
- 已新增质量快照趋势对比命令，可把两份快照对比为 JSON/Markdown 报告，并在严格模式下拦截解析质量退化。
- 已支持用 `--input` 对本地真实响应做一次性质量诊断，便于先验证、再脱敏沉淀为语料。
- 已新增 `perf:scheme` 核心解析性能预算，可基于脱敏语料构造 50KB / 250KB 响应，输出耗时、覆盖率、CMD 结构、资源字段、占位符、待检查和跳过数量，并支持严格模式拦截本地退化。
- 已新增 `perf:jsonpath` JSONPath 性能预算，可复用脱敏响应和大量命中列表，输出查询耗时、命中数、高亮范围和结果上限保护，并接入 CI 构建产物。
- 已新增匿名工具事件闭环，主工具会按功能名、状态、输入大小档和耗时档上报，管理后台可查看工具事件总量、失败率、高频功能、输入大小和耗时分布，默认不采集 JSON 原文。
- 根据真实响应对照发现并补齐字符串型资源 URL 洞察，`button_icon`、`user_portrait`、`button_image`、lottie 等不带查询参数的素材字段会进入资源字段和静态资源 URL Top。
- AI 修复发送前会默认阻断 `token`、`sign`、`cookie`、密钥和设备标识等疑似敏感字段，覆盖多层 URL 编码和内部 Base64 片段，避免把真实响应原文直接发给模型。
...
- 资源字段已补齐原始 URL 来源追踪，带查询参数的 `video_url`、`imageUrl` 等素材地址在展示为解析对象的同时仍能进入静态资源 URL Top。
...
- 质量快照趋势对比已支持资源类型可选阈值，能用 `--resource-type-drop video=20`、`--resource-type-rise lottie=20` 在严格模式下拦截视频占比骤降或 Lottie 占比异常上升。
- CI 已接入完整质量快照趋势对比，当前快照会和已提交基线做严格差异检查，并上传 `scheme-corpus-quality-trend` 构建产物，资源类型阈值会参与发布门禁。
- 已新增 `corpus:snapshot:baseline` 安全更新入口，严格检查通过后才覆盖已提交的质量趋势基线，并在 CI 文档中明确基线接受与回退标准。
- 已接入前端 ESLint 扁平配置，`npm run lint` 作为错误级门禁进入本地 CI 和 GitHub Actions，同时保留 `npm run lint:report` 用于逐步清理历史警告。
- 已从版本库移除 `frontend/.vite` 缓存文件，避免 Vite 本地预构建元数据继续污染代码提交。
- 已完成前端依赖安全治理，`npm audit` 从 7 个风险收敛为 0 个风险，并新增 `audit:security` 门禁拦截中危及以上漏洞回流。
...
- 管理后台工具事件洞察已补 PM 速览，直接展示高频功能、大输入占比、慢操作占比和建议动作，帮助判断“大响应是否卡顿”和“失败主要在哪”。
- 已新增第二类脱敏落地页响应语料，覆盖 easybrowse、deeplink、openapp、结构化 HTTPS 落地页、监测 URL、占位符和 cmdHandler 预期子集，质量快照样本数从 1 扩展到 2。
- 已新增第三类脱敏电话拨打响应语料，覆盖 makePhoneCall、numberUrl、logUrl、Base64 extInfo、hash 落地页参数、运行时占位符和 cmdHandler 预期子集，质量快照样本数从 2 扩展到 3。
...
- 深度解析报告已新增“真实响应下一步”动作条，根据当前报告推荐最多 3 个优先入口，把 cmdHandler 对比、占位符回填、待处理筛选、归档包和协作报告复制从长按钮区收敛到摘要下方。
- 已新增 `perf:e2e` 浏览器 Worker 端到端性能预算，覆盖 JSONPath 取消响应、Scheme 取消响应、连续大响应解析和已加载面板关闭态大输入切换，并在 CI 中输出 JSON 构建产物和步骤摘要。
...
- JSONPath 面板已补字段名快捷查询和 Response 常用查询预设，用户输入 `traceId` / `action_cmd` 会自动转成递归查询，`action_cmd`、`button_cmd`、`scheme`、`url`、`params`、`traceId` 也可一键填入并查询，减少真实接口响应排查时手写表达式的成本。
...
- HAR 派生载荷已新增问题导向摘要，统计客户端/服务端错误、未知状态、JSON 解析失败、截断正文和未解码 Base64，并只暴露不含查询参数的接口标签。
...
- 结构导航已补对象数组表格预览、全列搜索和稀疏字段重采样，选中列表型响应数组时可直接看前几行字段分布，按列名从受限扫描行中聚焦关键字段，并复制当前可见列的表格 JSON 或 CSV 继续排查。
...
- 结构导航已补搜索命中高亮，精确命中的字段名和值预览片段会在结构行内突出显示，降低大响应扫描成本。
...
- JSON 转 TS 已补生成可信度摘要，主转换和结构节点复制会说明样本数量、对象类型数量、可选字段、混合类型和空数组未知风险，让从响应到前端类型的结果更容易复核。
- JSON 转 TS 已从基础转换静态链路移到动态加载路径，小输入按需 import、大输入继续走 transform worker，避免低频类型生成和可信度摘要逻辑继续挤压首屏预加载预算。

## 2026-06-20 竞品复评补充

本轮继续参考 JSON Hero、JSON Crack、JSON Editor Online / jsoneditor、DevToys、quicktype、JSON Formatter、JSONLint 和 JSONGrid。整体判断不变: 项目不应该走“按钮越多越强”的路线，而应继续把复杂响应、CMD 与 Scheme 排查做成更短的智能路径。
...
- 大文件和本地隐私要持续前置: jsoneditor 强调大 JSON、多视图、修复和 Schema；DevToys、JSON Formatter 类工具强调离线或本地处理。项目应继续守住 Worker、懒加载、预加载门禁和“不上传原始响应”的产品承诺。
...
参考 JSON Hero、JSON Crack、JSON Editor Online / jsoneditor、Dadroit、jq playground、JSON Compare 和 JSON Formatter 后，当前项目已经覆盖通用 JSON 工作台的大部分基础能力，差异化优势仍在复杂响应、CMD 与 Scheme 解析。下一阶段不建议继续堆单点按钮，而应把“结构理解、修复解释、协作排查”做得更像一个检查器。
...
- CyberChef: 481 个操作和配方模式说明“可组合流水线”比单个按钮更有生命力；本项目已先落地 Response 排查工作流，把 JSON 内含 CMD/Scheme 的场景串到深度解析报告，后续再围绕“URL 解码 → JSON 修复 → 深度解析 → JSONPath/Schema”形成可保存、可复制的轻量配方，而不是一次性复制 CyberChef 的大而全。
...
- URL/查询参数解析要明确输入边界。FreeFormatter 会拆出协议、主机、路径、查询参数、片段等 URL 部件；EasyTools 明确要求完整 URL，重复查询参数名导出为数组；Jam 聚焦查询参数转 JSON。对应到本项目，普通 HTTPS 应继续按 URL 处理，不默认当业务 Scheme；只有自定义 Scheme 或携带可展开业务参数的 URL 才进入 CMD/Scheme 链路。
...
- 隐私承诺应该前置。JSON Crack、EasyTools、quicktype 都强调浏览器本地处理或不上传样本。本项目已适合把“本地解析、不上传原始响应、AI 发送前敏感阻断”放在设置、AI 修复和导出报告中更显眼的位置。

短期优先级建议:

1. P0: 继续强化 CMD/Scheme 边界判断和样本回归，尤其是普通 HTTPS、结构化 HTTPS、查询参数 JSON、URL 编码 JSON、多层转义之间的分类解释。
2. P1: 继续统一 Schema/TS 生成可信度摘要；当前已覆盖 Schema 对象/字段规模、required/可选字段、union、format 和数组采样，后续可扩展到多样本字段覆盖率。
3. P1: 扩展查询/解析配方；当前已先做 Response 排查工作流，后续支持保存“URL 解码 → JSON 修复 → 深度解析 → JSONPath/Schema”的常用排查链路。
...
- 配方工作流: 借鉴 CyberChef 的配方思路，沉淀常用排查链路，例如“URL 反解 → JSON 修复 → 深度解析 → 查询 action_cmd → 复制排查报告”，先以本地收藏和分享文本落地，不急于做后端协作。
...
- 数组表格视图: 已完成前 N 行对象数组表格、全列搜索、稀疏字段重采样和 CSV/JSON 复制；后续补展开嵌套对象和更多 CSV 选项，降低接口列表型响应的阅读成本。
...
- 更新 `ARCHITECTURE.md`，补 Worker、版本感知、质量快照、cmdHandler 差异和语料/性能门禁的当前实现。
- 继续补充纯监测链路或文件/HAR 导入样本，进一步降低语料的业务形态偏置。

### 第 2 周

- 继续优化“真实响应下一步”动作条的排序策略，结合工具事件观察用户实际点击后再决定是否加入更多业务规则。
- 继续观察浏览器 Worker 性能预算构建产物，结合真实慢操作事件决定是否把 70KB 连续解析扩展到 250KB 分层预算。
- 继续观察 PM 周报中的重点关注项，必要时按功能补充失败原因、取消率和慢操作样本分桶。
- 观察 JSON Schema 生成、校验和收藏共享的工具事件与用户反馈，再决定是否补充 Schema 差异、字段说明生成或团队规则包。

## 不建议马上做的事

- 不建议直接接入大量新格式编辑器。YAML/XML/HAR 应先以导入解析为主，避免扩大编辑和回写复杂度。
- 不建议立刻重写 Scheme 解析器。当前能力已覆盖真实响应，短期更应该用语料锁住行为，再逐步拆分。
- 不建议把管理后台做成重 BI。当前优先级应是轻量指标反哺产品决策。

## 下一步决策

如果只选一个方向继续投入，建议优先做“结构检查器 MVP”。解析质量语料和 CI 基线已经形成基础闭环，下一阶段应提升用户在大响应中定位、理解、复制和协作的效率。
