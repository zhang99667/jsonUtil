...
- 结构导航已补对象数组表格预览和列筛选，选中列表型 response 数组时可直接看前几行字段分布，按列名聚焦关键字段，并复制当前可见列的表格 JSON 或 CSV 继续排查。
...
- 数组表格视图: 已完成前 N 行对象数组表格、列筛选和 CSV/JSON 复制；后续补展开嵌套对象和更多 CSV 选项，降低接口列表型 response 的阅读成本。
