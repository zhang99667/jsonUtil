...
- 当前仓库版本: v1.8.170。
- 当前测试规模: Vitest 全量 42 个测试文件、759 个用例，E2E spec 3 个，前端源码文件约 120 个。
- 关键模块体量:
...
- HAR 请求提取: 打开 `.har` 文件时会生成不绑定原抓包文件句柄的派生 JSON 标签，提取 request/response body，补充 method/status/host/MIME 摘要和不含 query 的短标签，并默认进入深度解析。
...
- HAR 请求提取增强: 支持按域名、状态码和 MIME 类型筛选 payload，并导出选中的请求/响应片段。
