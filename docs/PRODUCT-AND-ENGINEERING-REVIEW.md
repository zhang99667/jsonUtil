...
- 当前仓库版本: v1.8.124。
- 当前测试规模: 前端单测文件 36 个，单测用例 711 个，E2E spec 3 个，前端源码文件约 106 个。
- 关键模块体量:
  - `frontend/src/utils/transformSummary.ts`: 3371 行
  - `frontend/src/utils/schemeUtils.ts`: 2380 行
  - `frontend/src/components/TransformReportPanel.tsx`: 2403 行
  - `frontend/src/components/SchemeViewerModal.tsx`: 1388 行
  - `frontend/src/App.tsx`: 1865 行
...
- 给资源 URL Top 增加“资源类型”分组，例如视频、图片、lottie、下载包，帮助 PM 更快判断素材问题分布。
