...
- 当前仓库版本: v1.8.171。
- 当前测试规模: Vitest 全量 42 个测试文件、760 个用例，E2E spec 3 个，前端源码文件约 120 个。
- 关键模块体量:
  - `frontend/src/utils/transformSummary.ts`: 3670 行
  - `frontend/src/utils/schemeUtils.ts`: 2380 行
  - `frontend/src/components/TransformReportPanel.tsx`: 2572 行
...
- 观察 JSON Schema 校验的工具事件和用户反馈，再决定是否补充 Schema 收藏、格式校验扩展或编辑器内错误装饰。
