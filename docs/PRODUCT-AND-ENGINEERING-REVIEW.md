...
- jq / JSONPath Plus / JSON Path Finder: 查询工具的核心价值是低门槛定位字段、复制路径和组合过滤；本次已先落地 JSONPath 字段名快捷查询，后续可以补路径点击生成查询、查询历史分组、jq/JMESPath 只读预览。
...
- 查询降门槛: JSONPath 字段名直查已落地，下一步可在结构导航节点上提供“复制递归字段查询”“复制精确路径查询”和“一键发送到 JSONPath 面板”，让不熟 JSONPath 的用户也能完成排查。
