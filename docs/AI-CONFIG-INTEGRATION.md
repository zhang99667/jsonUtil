...
| 工具入口 | `.claude/ai-tools-guide.md`、`.codex/README.md`、`.github/copilot-instructions.md`、`.cursorrules`、`.cursor/rules/**/*.mdc` | 针对不同 AI 工具提供薄入口，转发到权威规范 |
...
`.claude/ai-tools-guide.md` 是跨工具说明。它面向 Claude Code、Codex、GitHub Copilot、Cursor 等工具说明如何进入同一套规范。
...
- 必需 AI 协作资产是否存在。
- 入口文档是否引用权威规范和关键验证命令。
- AGENTS/CLAUDE 同源章节和 Cursor 薄入口是否漂移。
...
- `.claude/`、`.codex/`、`.cursor/rules/**/*.mdc`、MCP 配置（`.mcp.json`、`.cursor/mcp.json`、`.vscode/mcp.json`）、`.github/copilot-instructions.md`、`.github/instructions/**/*.instructions.md`、`.github/prompts/**/*.prompt.md`、`.github/agents/**/*.agent.md`、`.github/chatmodes/**/*.chatmode.md`、`docs/AI-*.md` 和 `rules/ai-*.md` 新增资产是否进入治理清单、引用规则或显式豁免。
