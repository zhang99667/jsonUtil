...
import { CLAUDE_SKILL_ADAPTERS, renderClaudeSkillAdapter } from './aiGovernanceClaudeSkillAdapters.mjs';
import { CODEX_PROJECT_COMMAND_RULE_FILES } from './aiGovernanceCodexCommandRules.mjs';
...
import { PROJECT_AI_INFRA_AUDITOR_ADAPTERS, renderProjectAiInfraAuditorAdapter } from './aiGovernanceProjectAiInfraAuditor.mjs';
...
const mirroredAgentSection = [
  '## AI 协作与子 Agent 委派',
  '',
  '- 跨模块排查、影响面分析、复杂重构或多条验证链路并行时，先判断是否需要子 Agent 委派；委派任务说明读写范围、排除项、期望输出和未覆盖风险，只读调查优先交给默认 `read-only` 的 explorer，限定写入交给 worker，构建/测试复核交给 verifier；profile sandbox 只是角色默认值，父任务权限可能覆盖。',
...
const writeMinimalGovernanceFixture = (rootDir) => {
  ...
  const sharedReferences = [
    ...
    'node scripts/ci/check-ai-asset-distribution.mjs --workspace', 'node scripts/ci/check-ai-asset-distribution.mjs --index', '使用 `--head`', 'CI 用 `--head`', 'AVAILABLE', '插件目录发现', '不会因 clone 或打开项目自动安装', 'node scripts/ci/manage-project-plugins.mjs --check', '新建任务', '.codex/config.toml', 'trusted project', '兼容插件', 'compatibility-plugin', 'fresh task', '兼容/可分发包', '新任务实际注册', '用户级插件 selector',
  ...
  writeFixtureFile(rootDir, 'scripts/ci/aiGovernanceProjectPluginLifecycle.mjs',
    "import './aiGovernanceProjectPluginCache.mjs';");
  writeFixtureFile(rootDir, 'scripts/ci/aiGovernanceProjectPluginLockWriter.mjs',
    "import './aiGovernanceProjectPluginLockSource.mjs';\nimport './aiGovernanceProjectPluginLockTransaction.mjs';");
  ...
  writeFixtureFile(rootDir, 'scripts/ci/aiGovernanceRegistrationCanarySnapshotPreflight.mjs',
    "import './aiGovernanceEvolutionSealedWorktreeManifest.mjs';\nimport './aiGovernanceRegistrationCanarySnapshotContract.mjs';\nimport './mcpLineDelimitedStdioClient.mjs';");
  ...
  writeFixtureFile(rootDir, 'scripts/ci/aiGovernanceEvolutionSealedWorktreeManifest.mjs',
    "import './aiGovernanceEvolutionSnapshotPrimitives.mjs';\nimport './aiGovernanceEvolutionSealedWorktreeManifestContract.mjs';");
  ...
  PROJECT_AI_INFRA_AUDITOR_ADAPTERS.forEach(descriptor => writeFixtureFile(rootDir, descriptor.file, renderProjectAiInfraAuditorAdapter(descriptor)));
  writeFixtureFile(
    rootDir,
    'scripts/ci/check-ai-governance.mjs',
    `import './aiGovernanceAssetDistribution.mjs';\nimport './aiGovernanceAssetDistributionReadiness.mjs';\nimport './aiGovernanceMaturityScorecardDistribution.mjs';\nimport './aiGovernanceClaudeSkillAdapters.mjs';\nimport './aiGovernanceClaudeSkillAdapterCaseDescriptors.mjs';\nimport './aiGovernanceCodexAgentProfiles.mjs';\nimport './aiGovernanceCodexAgentCaseDescriptors.mjs';\nimport './aiGovernanceProjectAiInfraAuditor.mjs';\nimport './aiGovernanceCodexCommandRules.mjs';\nimport './aiGovernanceCodexCommandRuleCaseDescriptors.mjs';\nimport './aiGovernanceCodexHooks.mjs';\nimport './aiGovernanceCodexHookCaseDescriptors.mjs';\nimport './aiGovernanceCodexProjectMcpConfig.mjs';\nimport './aiGovernanceEvolutionOutcomeChain.mjs';\nimport './aiGovernanceEvolutionTrialReceipts.mjs';\nimport './aiGovernanceEvolutionTrace.mjs';\nimport './aiGovernanceEvolutionTraceOutcomes.mjs';\nimport './aiGovernanceEvolutionTracePolicies.mjs';\nimport './aiGovernanceEvolutionTraceProof.mjs';\nimport './aiGovernanceCodexExecCaptureRuntime.mjs';\nimport './aiGovernanceCodexExecTraceAdapter.mjs';\nimport './aiGovernanceCodexExecTraceProjection.mjs';\nimport './aiGovernanceCodexFixedMcpTrialProfile.mjs';\nimport './aiGovernanceCodexFixedMcpTrialPreflight.mjs';\nimport './aiGovernanceCodexFixedMcpTrialCapture.mjs';\nimport './aiGovernanceCodexFixedMcpTrialLedger.mjs';\nimport './aiGovernanceCodexFixedMcpTrial.mjs';\nimport './aiGovernanceCodexExternalControllerTopology.mjs';\nimport './aiGovernanceCodexExternalControllerRuntimeProbe.mjs';\nimport './aiGovernanceEvolutionCodexCaseDescriptors.mjs';\nimport './aiGovernanceEvolutionFeedbackInbox.mjs';\nimport './aiGovernanceEvolutionExperiments.mjs';\nimport './aiGovernanceEvolutionLearningReport.mjs';\nimport './aiGovernanceEvolutionSuiteReport.mjs';\nimport './aiGovernanceRegistrationCanaryPacket.mjs';\nimport './aiGovernanceRegistrationCanaryResult.mjs';\nimport './aiGovernanceRegistrationCanaryReview.mjs';\nimport './aiGovernanceRequiredEvolutionLearningFiles.mjs';\nimport './aiGovernanceRequiredEvolutionFiles.mjs';\nimport './aiGovernanceProjectPlugins.mjs';\nimport './aiGovernanceRequiredProjectPluginFiles.mjs';\nimport './aiGovernanceRequiredValidationFiles.mjs';\n${sharedReferences}`
    .concat("\nimport './aiGovernanceCiContract.mjs';\nimport './aiGovernanceDecisionLedgerBackfillPathContract.mjs';\nimport './aiGovernanceCodexExecJsonlFraming.mjs';\nimport './aiGovernanceRequiredMcpFiles.mjs';\nimport './aiGovernanceEvolutionCaseRunner.mjs';\nimport './aiGovernanceEvolutionExecutableCases.mjs';\nimport './aiGovernanceEvolutionCaseFailure.mjs';\nimport './aiGovernanceRequiredEvolutionRunnerFiles.mjs';\nimport './aiGovernanceProjectPluginCommand.mjs';"));
  writeFixtureFile(rootDir, 'scripts/ci/run-ai-codex-fixed-mcp-trial.mjs', "import('./aiGovernanceCodexFixedMcpTrialCli.mjs');");
  [['scripts/ci/run-ai-validation-execution.mjs', "import './aiGovernanceValidationExecution.mjs';"], ['scripts/ci/check-ai-validation-whitespace.mjs', "import './aiGovernanceValidationWhitespace.mjs';"], ['scripts/ci/aiGovernanceCiContract.mjs', "import './aiGovernanceCiEntrySource.mjs';\nimport './aiGovernanceOutcomeWriterAutomationContract.mjs';"], ['scripts/ci/aiGovernanceCiEntrySource.mjs', "import './aiGovernanceStableUtf8File.mjs';"], ['scripts/ci/aiGovernanceValidationExecution.mjs', "import './aiGovernanceValidationExecutionState.mjs';\nimport './aiGovernanceValidationExecutables.mjs';\nimport './aiGovernanceValidationRuntime.mjs';\nimport './aiGovernanceValidationWorkspaceRuntime.mjs';"], ['scripts/ci/aiGovernanceValidationExecutionState.mjs', "import '../mcp/jsonutils-governance-validation-plan.mjs';\nimport './aiGovernanceCodexFixedMcpTrialLedger.mjs';\nimport './aiGovernanceEvolutionWorktreeRevision.mjs';\nimport './aiGovernanceValidationChangedSet.mjs';\nimport './aiGovernanceValidationCommandRegistry.mjs';\nimport './aiGovernanceValidationWorkspaceRuntime.mjs';"], ['scripts/ci/aiGovernanceValidationChangedSet.mjs', "import './aiGovernanceValidationChangedSetGitInventory.mjs';\nimport './aiGovernanceValidationRuntimePrimitives.mjs';"], ['scripts/ci/aiGovernanceValidationChangedSetGitInventory.mjs', "import './aiGovernanceHermeticGitInventory.mjs';\nimport './aiGovernanceValidationRuntimePrimitives.mjs';"], ['scripts/ci/aiGovernanceValidationExecutables.mjs', "import './aiGovernanceValidationRuntimePrimitives.mjs';"], ['scripts/ci/aiGovernanceValidationRuntime.mjs', "import './aiGovernanceValidationRuntimePrimitives.mjs';"], ['scripts/ci/aiGovernanceValidationWorkspaceRuntime.mjs', "import './aiGovernanceValidationRuntimePrimitives.mjs';"], ['scripts/ci/aiGovernanceValidationWhitespace.mjs', "import './aiGovernanceValidationChangedSet.mjs';\nimport './aiGovernanceValidationChangedSetGitInventory.mjs';\nimport './aiGovernanceValidationRuntimePrimitives.mjs';"], ['scripts/ci/aiGovernanceAssetDistribution.mjs', "import './aiGovernanceAssetDistributionFiles.mjs';\nimport './aiGovernanceAssetDistributionGitEvidence.mjs';"], ['scripts/ci/aiGovernanceAssetDistributionFiles.mjs', "import './aiGovernanceImplementationFiles.mjs';"], ['scripts/ci/record-ai-evolution-deterministic-outcomes.mjs', "import './aiGovernanceEvolutionDeterministicOutcomeWriter.mjs';"], ['scripts/ci/aiGovernanceEvolutionDeterministicOutcomeWriter.mjs', "import './aiGovernanceEvolutionDeterministicOutcomeCandidate.mjs';\nimport './aiGovernanceEvolutionDeterministicOutcomeTransaction.mjs';"], ['scripts/ci/aiGovernanceEvolutionDeterministicOutcomeCandidate.mjs', "import './aiGovernanceEvolutionOutcomeChain.mjs';\nimport './aiGovernanceEvolutionTrialReceipts.mjs';"], ['scripts/ci/aiGovernanceEvolutionDeterministicOutcomeCandidate.test.mjs', "import './aiGovernanceEvolutionDeterministicOutcomeCandidate.mjs';\ntest('candidate', () => {});"], ['scripts/ci/aiGovernanceEvolutionDeterministicOutcomeTransaction.mjs', "import './aiGovernanceEvolutionOutcomeRecoveryResult.mjs';\nimport './aiGovernanceEvolutionOutcomeTransactionContract.mjs';\nimport './aiGovernanceEvolutionOutcomeTransactionFailure.mjs';"], ['scripts/ci/aiGovernanceEvolutionTrialReceipts.mjs', "import './aiGovernanceEvolutionPairedReceiptV4BatchContract.mjs';\nimport './aiGovernanceEvolutionTrialReceiptSource.mjs';\nimport './aiGovernanceEvolutionTrialReceiptContract.mjs';"], ['scripts/ci/aiGovernanceEvolutionFeedbackInbox.mjs', "import './aiGovernanceEvolutionFeedbackProfiles.mjs';\nimport './aiGovernanceEvolutionFeedbackSource.mjs';"], ['scripts/ci/record-ai-evolution-unverified-trace-outcome.mjs', "import './aiGovernanceEvolutionUnverifiedTraceOutcomeWriter.mjs';"], ['scripts/ci/aiGovernanceEvolutionUnverifiedTraceOutcomeWriter.mjs', "import './aiGovernanceEvolutionUnverifiedTraceObservationContract.mjs';"], ['scripts/ci/aiGovernanceEvolutionUnverifiedTraceOutcomeWriter.test.mjs', "import './aiGovernanceEvolutionUnverifiedTraceOutcomeWriterTestFixtures.mjs';"], ['scripts/ci/record-ai-evolution-paired-outcome.mjs', "import './aiGovernanceEvolutionPairedReceiptV4BatchContract.mjs';\nimport './aiGovernanceEvolutionPairedOutcomeWriter.mjs';"], ['scripts/ci/aiGovernanceEvolutionPairedOutcomeWriter.mjs', "import './aiGovernanceEvolutionPairedReceiptV4.mjs';\nimport './aiGovernanceEvolutionPairedOutcomeCandidate.mjs';"], ['scripts/ci/aiGovernanceEvolutionPairedOutcomeCandidate.mjs', "import './aiGovernanceEvolutionPairedReceiptV4.mjs';\nimport './aiGovernanceEvolutionPairedReceiptV4BatchContract.mjs';\nimport './aiGovernanceEvolutionPairedReceiptV4Commitments.mjs';"], ['scripts/ci/aiGovernanceEvolutionPairedReceiptV4Proof.mjs', "import './aiGovernanceEvolutionPairedReceiptV4Commitments.mjs';"], ['scripts/ci/aiGovernanceEvolutionPairedReceiptV4.mjs', "import './aiGovernanceEvolutionPairedReceiptV4Assignment.mjs';\nimport './aiGovernanceEvolutionPairedReceiptV4BatchContract.mjs';\nimport './aiGovernanceEvolutionPairedReceiptV4Commitments.mjs';\nimport './aiGovernanceEvolutionPairedReceiptV4Proof.mjs';"], ['scripts/ci/aiGovernanceEvolutionPairedReceiptV4BatchContract.mjs', "import './aiGovernanceEvolutionEvalContract.mjs';\nimport './aiGovernanceEvolutionPairedReceiptV4Assignment.mjs';\nimport './aiGovernanceRegistrationCanaryDsseEnvelope.mjs';"], ['scripts/ci/aiGovernanceEvolutionPairedReceiptV4BatchContract.test.mjs', "import './aiGovernanceEvolutionPairedReceiptV4BatchContract.mjs';\nimport './aiGovernanceEvolutionPairedReceiptV4TestFixtures.mjs';\ntest('batch contract', () => {});"], ['scripts/ci/aiGovernanceEvolutionPairedReceiptV4Commitments.test.mjs', "import './aiGovernanceEvolutionPairedReceiptV4Commitments.mjs';\ntest('commitments', () => {});"], ['scripts/ci/aiGovernanceEvolutionPairedOutcomeCandidate.test.mjs', "import './aiGovernanceEvolutionPairedOutcomeCandidate.mjs';\ntest('candidate', () => {});"], ['scripts/ci/aiGovernanceEvolutionPairedOutcomeCli.test.mjs', "import './aiGovernanceEvolutionPairedReceiptV4TestFixtures.mjs';\ntest('fixture', () => {});"]].forEach(([file, source]) => writeFixtureFile(rootDir, file, source));
  writeFixtureFile(rootDir, 'scripts/ci/aiGovernanceValidationExecution.mjs',
    "import './aiGovernanceValidationExecutionState.mjs';\nimport './aiGovernanceValidationExecutables.mjs';\nimport './aiGovernanceValidationRuntime.mjs';\nimport './aiGovernanceValidationWorkspaceRuntime.mjs';\nimport './aiGovernanceValidationExecutionReceipt.mjs';");
  writeFixtureFile(rootDir, 'scripts/ci/aiGovernanceValidationExecutionState.test.mjs',
    "import './aiGovernanceValidationExecutionState.mjs';\ntest('state', () => {});");
  writeFixtureFile(rootDir, 'scripts/ci/aiGovernanceValidationExecution.test.mjs',
    "import './aiGovernanceValidationExecutionReceipt.mjs';\nimport './aiGovernanceValidationExecutionTestFixtures.mjs';\ntest('execution', () => {});");
  ...
  writeFixtureFile(rootDir, '.agents/skills/jsonutils-ai-infra-evolver/evals/evals.json', fs.readFileSync(
    new URL('../../.agents/skills/jsonutils-ai-infra-evolver/evals/evals.json', import.meta.url),
    'utf8',
  ));
  ...
  ['README.md', 'cases.json', 'grader-calibration.json'].forEach(file => writeFixtureFile(rootDir, `evals/ai-governance/${file}`,
    fs.readFileSync(new URL(`../../evals/ai-governance/${file}`, import.meta.url), 'utf8')));
  ...
  AI_ENTRY_SHARED_SNIPPET_FILES.forEach((file) => {
    writeFixtureFile(rootDir, file, [sharedReferences, ...AI_ENTRY_SHARED_SNIPPETS,
      'plugins/jsonutils-governance-mcp@0.2.2', 'plugins/codex-mcp-config-auditor@0.2.2'].join('\n'));
  });
  ...
  writeFixtureFile(rootDir, 'docs/AI-EVOLUTION-PLAYBOOK.md', fs.readFileSync(
    new URL('../../docs/AI-EVOLUTION-PLAYBOOK.md', import.meta.url), 'utf8',
  ));
  ...
  CLAUDE_SKILL_ADAPTERS.forEach((descriptor) => writeFixtureFile(
    rootDir,
    descriptor.adapterFile,
    renderClaudeSkillAdapter(descriptor, fs.readFileSync(path.join(rootDir, descriptor.canonicalFile), 'utf8')),
  ));
  ...
  syncTracePolicyFixture(rootDir, new URL('../../', import.meta.url));
...
test('AI 治理规则构造会展开 skill 路径和发布资源关键词', () => {
  ...
  const thinEntryRules = [claudeRule, codexRule, copilotRule, cursorRule];
  ...
  [
    'scripts/ci/aiGovernanceAssetDistributionReadiness.mjs', 'scripts/ci/aiGovernanceAssetDistributionReadiness.test.mjs',
    'scripts/ci/aiGovernanceMaturityScorecardDistribution.mjs', 'scripts/ci/aiGovernanceMaturityScorecardDistribution.test.mjs',
  ].forEach(file => assert.equal(requiredFiles.includes(file), true));
  ...
  [claudeRule, codexRule, copilotRule, cursorRule, playbookRule, skillRule].forEach((rule) => {
