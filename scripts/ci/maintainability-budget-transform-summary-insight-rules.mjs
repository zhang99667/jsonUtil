...
export const transformSummaryInsightMaintainabilityBudgets = [
  ...
  summaryInsightBudget('frontend/src/utils/transformReportCmdStructureSource.ts', 65, '深度解析 CMD 结构源入口只负责记录筛选、schema 提取和结构源组合'),
  summaryInsightBudget('frontend/src/utils/transformReportCommandParamSummary.ts', 35, '深度解析 CMD 参数摘要只负责顶层 key 计数与展示上限'),
  summaryInsightBudget('frontend/src/utils/transformReportCmdStructureCopyText.ts', 35, '深度解析 CMD 结构复制文本只负责聚焦路径和 cmdHandler 兼容格式'),
