...
export const schemeSupportQueryMaintainabilityBudgets = [
  ...
  schemeSupportQueryBudget('schemeQueryDecoding.ts', 50, 'Scheme query 编解码应集中处理表单加号和 URL Decode 失败兜底，避免回流核心解码流程'),
  schemeSupportQueryBudget('schemePrefixedQuery.ts', 60, '带日志前缀的 query 参数串切分应保持纯函数，避免 Scheme 主流程和元信息导出规则漂移'),
  schemeSupportQueryBudget('schemePlaceholders.ts', 120, '运行时占位符识别、迭代收集和分组规则应保持集中、可审计'),
  schemeSupportQueryBudget('schemePlaceholdersDeepTraversal.test.ts', 25, '运行时占位符测试锁定七千层对象数组混合链的稳定收集'),
