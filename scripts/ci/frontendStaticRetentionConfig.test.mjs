...
const externalAdminLocation = [
  '    location = /admin.html {',
  '        try_files /admin.html /index.html =404;',
  '        add_header Clear-Site-Data "\\"cache\\"" always;',
  '        sub_filter \'<head>\' \'<head><script>(function(){if(location.pathname==="/admin.html"){history.replaceState(null,document.title,"/");}})();</script>\';',
  '    }',
  '',
].join('\n');
...
${externalAdminLocation}    location / { try_files $uri $uri/ /index.html; }
}
...
test('Nginx 公开域名路由检查会拦截外部业务域名缺少后台入口隔离', () => {
  withTempRoot((rootDir) => {
    ...
    assert.deepEqual(collectNginxPublicRoutingFailures(rootDir), [
      'frontend/nginx.conf: 外部域名 zhangjihao.markz.fun 必须直出 /admin.html 或本域 /index.html，并把历史后台路径归位到裸域',
    ]);
  });
});
  ...
  withTempRoot((rootDir) => {
    writeFixtureFile(
      ...
      nginxConfig(
        'jsonutils.markz.fun markz.fun www.markz.fun',
        'jsonutils.markz.fun markz.fun www.markz.fun'
      ).replace('    location = / { try_files /index.html =404; }\n', '')
    );
