...
import { collectRegistryClassificationCombinationFailures } from './aiGovernanceAssetRegistryClassificationCombinations.mjs';
...
export const collectRegistryClassificationFailures = (file, row) => {
  ...
  return [
    ...collectFutureIsoDateFailures(prefix(file), '最近复核日期', row.reviewDate),
    ...collectRegistryClassificationCombinationFailures(file, row),
  ];
};
