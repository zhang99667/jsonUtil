...
export const governanceAppWorkflowCommandMaintainabilityBudgets = [
  governanceCommandBudget('scripts/ci/maintainability-budget-app-workflow-command-rules.mjs', 25, 'App 命令工作流预算入口应只组合 core、智能建议、Scheme、设置备份、模板填充和工具面板子表'),
  governanceCommandBudget('scripts/ci/maintainability-budget-app-workflow-command-core-rules.mjs', 25, 'App 命令核心预算规则应只组合自动保存子表并维护复制、主动作和面板布局预算'),
  governanceCommandBudget('scripts/ci/maintainability-budget-app-workflow-command-smart-suggestion-rules.mjs', 15, '智能建议命令预算规则应独立维护 hook、runner 和计划副作用预算'),
