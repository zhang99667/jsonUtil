...
export const governanceAppShellMaintainabilityBudgets = [
  ...
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-file-system-rules.mjs', 10, 'App 文件系统预算入口应只组合命令、运行时和测试子表'),
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-file-system-command-rules.mjs', 10, 'App 文件系统命令预算规则应只维护打开与保存事务预算'),
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-file-system-runtime-rules.mjs', 15, 'App 文件系统运行时预算规则应只维护 hook 和纯 helper 预算'),
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-file-system-hook-test-rules.mjs', 15, 'App 文件系统 hook 测试预算规则应独立维护工作流 hook 和共享测试状态预算'),
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-file-system-test-rules.mjs', 15, 'App 文件系统测试预算规则应只组合 hook 测试并维护纯 helper 对应测试预算'),
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-workspace-draft-rules.mjs', 8, '工作区草稿预算子表只维护运行时与对应回归测试预算'),
  ...
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-interaction-hook-rules.mjs', 20, 'App 交互基础 hook 预算规则应只组合 PREVIEW setter、RAF、首次引导、telemetry 和 scrollbar 子表'),
  ...
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-json-schema-example-rules.mjs', 15, 'Schema 示例预算规则应维护主入口、原始值、数值和动态属性名模块及测试预算'),
  governanceAppShellBudget('scripts/ci/maintainability-budget-app-json-compare-rules.mjs', 10, 'JSON 语义对比预算规则应只维护实现和直接测试预算'),
