...
export const governanceAiEvolutionMaintainabilityBudgets = [
  evolutionBudget('scripts/ci/maintainability-budget-governance-ai-evolution-rules.mjs', 55, 'AI evolution 预算子表应只维护 corpus、receipt、trace、outcome 与 replay 数据面'),
  ...
  evolutionBudget('scripts/ci/aiGovernanceEvolutionJsonlSource.mjs', 85, 'AI evolution JSONL source 应只维护稳定有界普通文件、fatal UTF-8/BOM 与物理 framing，不承载领域 schema 或 trust 语义'),
  evolutionBudget('scripts/ci/aiGovernanceEvolutionOutcomeLedger.mjs', 120, 'AI evolution outcome ledger 应只消费共享 JSONL source，并编排 outcome 解析、chain、日期单调性与 ID 唯一性'),
  ...
  evolutionBudget('scripts/ci/aiGovernanceEvolutionDeterministicOutcomeCandidate.mjs', 200, 'AI evolution deterministic candidate 应只从 runner 结果、当前 revision 与双 ledger 尾部派生 receipt/outcome suffix'),
  evolutionBudget('scripts/ci/aiGovernanceEvolutionDeterministicOutcomeWriter.mjs', 390, 'AI evolution deterministic writer 应只编排固定 runner、安全 ledger snapshot、候选重放与事务提交'),
  evolutionBudget('scripts/ci/aiGovernanceEvolutionDeterministicOutcomeTransaction.mjs', 410, 'AI evolution deterministic transaction 应只维护协作锁、持久追加与崩溃恢复执行'),
  evolutionBudget('scripts/ci/aiGovernanceEvolutionOutcomeTransactionContract.mjs', 145, 'AI evolution transaction contract 应只维护 journal 闭字段、身份、字节边界与 digest 绑定'),
  ...
  evolutionBudget('scripts/ci/aiGovernanceEvolutionTrialReceiptContract.mjs', 220, 'AI evolution receipt contract 应只维护单条 v1-v4 字段、trial/validation、trace/proof 与 deterministic runner 契约'),
  evolutionBudget('scripts/ci/aiGovernanceEvolutionTrialReceiptSource.mjs', 60, 'AI evolution receipt source 应只消费共享 JSONL framing 并维护 receipt 物理记录解析'),
  evolutionBudget('scripts/ci/aiGovernanceEvolutionTrialReceipts.mjs', 130, 'AI evolution receipt ledger 应只编排稳定 source、单条 contract、ID 唯一索引与跨 receipt 重放拒绝'),
  ...
  ...[
    ['scripts/ci/aiGovernanceRequiredEvolutionFiles.mjs', 70, 'AI evolution 必需资产清单应只组合 runner、writer、trace/framing、proof、runtime probe 与对应测试'],
