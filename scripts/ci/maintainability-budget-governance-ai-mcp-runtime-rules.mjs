...
export const governanceAiMcpRuntimeMaintainabilityBudgets = [
  ...
  mcpRuntimeBudget('scripts/mcp/jsonutils-governance-jsonrpc.mjs', 45, 'JSONUtils 治理 MCP JSON-RPC core 应只维护通用请求 envelope、notification 识别和标准错误载荷'),
  mcpRuntimeBudget('scripts/mcp/jsonutils-governance-method-params.mjs', 45, 'JSONUtils 治理 MCP method params helper 应单源维护固定方法参数 shape 与 Invalid params 类型'),
  ...
  mcpRuntimeBudget('scripts/mcp/jsonutils-governance-session.mjs', 80, 'JSONUtils 治理 MCP session helper 应单源维护 session admission、初始化状态与有序控制消息，并把 ready 请求委派给 lifecycle'),
