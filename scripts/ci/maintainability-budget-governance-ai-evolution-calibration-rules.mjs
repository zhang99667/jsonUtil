...
export const governanceAiEvolutionCalibrationMaintainabilityBudgets = [
  ...
  calibrationBudget('scripts/ci/aiGovernanceRegistrationCanaryCalibrationFixtures.mjs', 190, 'Registration calibration context fixture 只构造脱敏 packet/result 与独立 operation-ID，不包含 oracle 或 mutation 路由'),
  calibrationBudget('scripts/ci/aiGovernanceRegistrationCanaryCalibrationMutations.mjs', 100, 'Registration calibration mutation 叶子只将 17 类固定 mutation 投影为 grader 输入，不读取仓库或调用 grader'),
  calibrationBudget('scripts/ci/aiGovernanceRegistrationCanaryCalibrationMutations.test.mjs', 70, 'Registration calibration mutation 直接测试锁 facade 同引用、control/behavior 与 digest 对抗边界'),
  ...
  calibrationBudget('scripts/ci/aiGovernanceRegistrationCanaryGraderCalibrationRedteam.test.mjs', 150, 'Registration grader calibration 红队应锁双 fixture 源路径替换、祖先 symlink、ID/taxonomy 缩减与 operation-ID 同源自证'),
];
