...
export const transformPanelHelperUiMaintainabilityBudgets = [
  ...
  panelUiBudget('frontend/src/utils/transformReportPanelCopyTitleState.ts', 60, '深度解析报告复制标题状态 helper 应只把派生模型状态转换为 title 矩阵输入'),
  panelUiBudget('frontend/src/utils/transformReportPanelCopyTitleState.test.ts', 75, '深度解析报告复制标题状态测试只锁定布尔矩阵映射和标题矩阵复用'),
  panelUiBudget('frontend/src/utils/transformReportPanelDerivedModel.ts', 80, '深度解析报告派生模型应只组合复制可用性、问题样本、占位符、质量快照和标题文案，状态与副作用留在主面板'),
  panelUiBudget('frontend/src/utils/transformReportPanelDerivedModelTypes.ts', 45, '深度解析报告派生模型类型契约应独立维护，避免组合 helper 被输入输出声明撑大'),
