...
test('检查固定 whitespace 语义且不执行仓库 filter 或 attribute driver', async (t) => {
  const rootDir = createRepository(t, {
  ...
  git(rootDir, ['config', 'diff.hostile.command', filter]);
  git(rootDir, ['config', 'diff.hostile.textconv', filter]);
...
test('超大工作树文件在整体读取前 fail closed 且不泄露路径', async (t) => {
  const rootDir = createRepository(t, { 'baseline.txt': 'ok\n' });
  const file = path.join(rootDir, 'oversized-secret.txt');
  ...
  assert.equal(report.ok, false);
  assert.deepEqual(report.blockers, [{ code: 'RAW_VIEW_SIZE_LIMIT_EXCEEDED', count: 1 }]);
  assert.equal(report.claims.applicableRawComparisonsCompleted, false);
  assert.equal(JSON.stringify(report).includes('oversized-secret.txt'), false);
});
...
test('CLI help 零读取，未知或重复参数为 2，检查失败为 1', async () => {
  ...
  const nestedCode = await runAiValidationWhitespaceCli({
    args: ['--json'], stdout: nestedOut.stream, stderr: output().stream,
    check: async () => ({
      schemaVersion: 1, reportType: 'ai-governance-validation-whitespace',
      profile: 'raw-head-index-worktree-whitespace-v1', ok: true, status: 'passed',
      evidenceScope: 'component-only', outcomeEligible: false,
      changedSet: { stateSha256: 'a'.repeat(64), changedFileCount: 0, path: '/secret/private/path' },
      checks: {
      ...
      blockers: [], claims: {
        applicableRawComparisonsCompleted: true, launcherShellUsed: false,
        repositoryFiltersExecuted: false, repositoryAttributeDriversExecuted: false,
        commandOutputReported: false, behaviorValidated: false,
      },
    }),
  });
  assert.equal(nestedCode, 1);
  assert.deepEqual(JSON.parse(nestedOut.read()), expectedFailure);
  assert.equal(nestedOut.read().includes('/secret/private/path'), false);
