...
export const schemeViewerDetailComponentMaintainabilityBudgets = [
  ...
  schemeViewerDetailComponentBudget('SchemeViewerParamSectionsPanel.test.tsx', 55, 'Scheme 参数来源面板测试只覆盖空态、计数、展示上限和参数预览截断'),
  ...
  schemeViewerDetailComponentBudget('SchemeViewerParamStagesPanel.test.tsx', 80, 'Scheme 参数分层面板测试只覆盖空态、链路摘要、修复提示、展示上限和可回写状态'),
  schemeViewerDetailComponentBudget('SchemeViewerDecodeLayersPanel.tsx', 120, 'Scheme 解码链路面板只负责层级证据展示，层级生成和编码可逆性规则留在解析层'),
  schemeViewerDetailComponentBudget('SchemeViewerDecodeLayersPanel.test.tsx', 85, 'Scheme 解码链路面板测试只覆盖空态、目标类型、层级展示和 after 内容 fallback'),
