...
export const governanceAiValidationTestMaintainabilityBudgets = [
  validationTestBudget('scripts/ci/maintainability-budget-governance-ai-validation-test-rules.mjs', 22, 'Validation 测试预算子表应只维护 changed-set/Git inventory、registry、runtime、executor、whitespace 与 plan 测试'),
  validationTestBudget('scripts/ci/aiGovernanceValidationChangedSet.test.mjs', 300, 'Validation changed-set 集成测试应锁定 Git flag、mode、symlink、filter、ignore、原始摘要与精确根负例'),
  validationTestBudget('scripts/ci/aiGovernanceValidationChangedSetGitInventory.test.mjs', 65, 'Validation Git inventory 直测应锁定专职导出面、NUL/UTF-8 parser 与路径脱敏失败'),
  ...
  validationTestBudget('scripts/ci/aiGovernanceValidationExecutionState.test.mjs', 120, 'Validation state 测试应锁定固定摘要、ledger 顺序、逐轴漂移、integrity 单调与路径脱敏'),
  validationTestBudget('scripts/ci/aiGovernanceValidationExecutionReceipt.test.mjs', 40, 'Validation receipt 测试应直连共享失败报告与闭字段校验'),
  validationTestBudget('scripts/ci/aiGovernanceValidationExecutionTestFixtures.mjs', 145, 'Validation executor 测试 fixture 应单源维护 state provider、changed-set、plan、registry、binding 与 runtime 夹具'),
