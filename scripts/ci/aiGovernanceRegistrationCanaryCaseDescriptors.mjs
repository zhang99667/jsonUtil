...
export const AI_EVOLUTION_REGISTRATION_CANARY_CASES = Object.freeze({
  ...
  'mcp-registration-canary-result-ingestion-boundary': {
    ...
    evidenceScope: 'component-only',
    evidence: ['闭字段 result、双 fixture 源摘要/固定 ID/完整 taxonomy/独立 operation-ID 的 grader calibration、observable trace 复用、blind grade set、host-only unblind、未验信指标与零自动写入负例'],
    argsList: [nodeTest('scripts/ci/aiGovernanceRegistrationCanaryBlindResult.test.mjs',
