...
export const governanceAiEvolutionTestMaintainabilityBudgets = [
  ...
  evolutionTestBudget('scripts/ci/aiGovernanceEvolutionDeterministicOutcomeCandidate.test.mjs', 125, 'AI evolution deterministic candidate 直接测试应锁定 ID、revision、chain、supersession、双 suffix 与复用边界'),
  evolutionTestBudget('scripts/ci/aiGovernanceEvolutionDeterministicOutcomeWriter.test.mjs', 350, 'AI evolution deterministic writer 测试应锁定 preview、漂移拒绝、幂等、CI 禁写与真实本地事务'),
  evolutionTestBudget('scripts/ci/aiGovernanceEvolutionDeterministicOutcomeTransaction.test.mjs', 280, 'AI evolution deterministic transaction 测试应锁定 lock/journal 所有权、恢复状态机与 fail-closed 执行边界'),
  evolutionTestBudget('scripts/ci/aiGovernanceEvolutionOutcomeTransactionContract.test.mjs', 120, 'AI evolution transaction contract 测试应锁定精确 journal 向量、闭字段、canonical base64、digest 与大小边界'),
