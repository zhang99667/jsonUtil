...
test('anchor observation set 按签名 proof 去重并拒绝真实可观察分叉', () => {
  ...
  const reversed = verifyRegistrationCanaryAnchorReceiptSet({
    checkpointRequestJson: fixture.checkpointRequestJson,
    anchorReceiptJsons: [JSON.stringify(renamedEnvelope), fixture.anchorReceiptJson],
  });
  assert.deepEqual(reversed, repeated);
  assert.equal(repeated.anchorRef.stateVersion, 1);
  const secondStatement = structuredClone(fixture.anchorStatement);
  secondStatement.predicate.batchId = `batch-${'b'.repeat(32)}`;
  secondStatement.predicate.anchorKeySha256 = buildRegistrationCanaryAnchorStatement({
    checkpointRequestJson: fixture.checkpointRequestJson,
    batchId: secondStatement.predicate.batchId,
    authority: secondStatement.predicate.authority,
    controller: secondStatement.predicate.controller,
  }).predicate.anchorKeySha256;
  const secondReceipt = fixture.signStatement(secondStatement, fixture.signers.anchor);
  assert.throws(() => verifyRegistrationCanaryAnchorReceiptSet({
    checkpointRequestJson: fixture.checkpointRequestJson,
    anchorReceiptJsons: [fixture.anchorReceiptJson, secondReceipt],
  }), /分叉|非幂等/);
});
