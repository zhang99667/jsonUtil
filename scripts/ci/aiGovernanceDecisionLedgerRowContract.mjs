...
import {
  ...
  extractNodeCommandPaths,
  extractNodeRegressionTestCommandPaths,
  isCiCoveredNodeRegressionTestPath,
} from './aiGovernanceDecisionLedgerCommandReferences.mjs';
...
export const collectDecisionLedgerRowFailures = (rootDir, row, index, ledgerFile) => {
  ...
  const executableCommands = extractExecutableCommands(row['锁定测试']);
  const regressionTestCommands = extractNodeRegressionTestCommandPaths(row['锁定测试']);
  const ciCoveredRegressionTestCommands = regressionTestCommands.filter(isCiCoveredNodeRegressionTestPath);
  ...
  return [
    ...
    ...regressionTestCommands.filter(file => !isCiCoveredNodeRegressionTestPath(file))
