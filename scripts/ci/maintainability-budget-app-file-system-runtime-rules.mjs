...
export const appFileSystemRuntimeMaintainabilityBudgets = [
  ...
  fileSystemRuntimeBudget('frontend/src/hooks/useFileAutoSave.ts', 65, '文件自动保存 hook 只维护活动文件快照、防抖调度、卸载代次和写入反馈'),
  fileSystemRuntimeBudget('frontend/src/hooks/useFileOpenRequestGuard.ts', 59, '文件打开请求守卫以单一内部状态源维护请求次序和工作区意图版本，不读取文件或修改标签状态'),
  fileSystemRuntimeBudget('frontend/src/hooks/useWorkspaceDraftPersistence.ts', 120, '工作区草稿持久化 hook 只维护恢复、防抖、隐藏页和卸载保存生命周期'),
  fileSystemRuntimeBudget('frontend/src/utils/browserFileSave.ts', 35, '浏览器下载 helper 只维护临时链接、Blob 和对象 URL 的容错回收'),
