...
import {
  ...
  findLatestEvolutionDeterministicLineageOutcome,
...
export const prepareEvolutionDeterministicOutcomeBatch = ({
  ...
  const pending = selected.filter(({ caseId, descriptor }) => {
    const previous = findLatestEvolutionDeterministicLineageOutcome(
      ledgers.outcomeResult.validOutcomes,
      { caseId, caseVersion: descriptor.caseVersion, subjectVersion: descriptor.subjectVersion },
    );
    if (!isEvolutionDeterministicPassReusable({
      previous,
  ...
  const candidates = buildEvolutionDeterministicOutcomeCandidates({
    selected: pending,
    resultsById,
    revision: revisionBefore,
    evaluatedAt,
  ...
  const candidateReport = validateCandidate({
    rootDir: realRoot,
    receiptsBytes: Buffer.concat([receiptsBefore.bytes, candidates.receiptSuffix]),
    outcomesBytes: Buffer.concat([outcomesBefore.bytes, candidates.outcomeSuffix]),
    evaluatedAt,
    revision: revisionBefore,
  });
  ...
  return {
    ...
    transaction: {
      ...
      receiptSuffix: candidates.receiptSuffix,
      outcomeSuffix: candidates.outcomeSuffix,
    },
  };
};
