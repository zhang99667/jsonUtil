...
const missingReferenceCases = [...AI_GOVERNANCE_REFERENCE_CORE_MISSING_CASES, ...AI_GOVERNANCE_REFERENCE_ENTRY_MISSING_CASES];
...
test('文档引用规则直连权威清单且不经过兼容代理层', () => {
  ...
  const consumers = ['aiGovernanceEntryCoreReferenceGroups.mjs', 'aiGovernanceRuntimeReferenceGroups.mjs', 'aiGovernancePlaybookSectionRules.mjs', 'aiGovernanceAssetRegistryEvidenceSources.mjs'];
  const source = consumers.map(file => readFileSync(new URL(file, import.meta.url), 'utf8')).join('\n');
  assert.doesNotMatch(source, /aiGovernance(?:ReferenceGroups|CollaborationReferenceGroups|AiBoundaryReferenceGroups)\.mjs/);
});
