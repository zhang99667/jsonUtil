...
export const SCORECARD_CANDIDATE_MAX_REMAINING_LINES = 5;
export const SCORECARD_CANDIDATE_MIN_RATIO = 0.85;
...
const isScorecardCandidate = usage => usage.lineCount <= usage.maxLines && (
  getRemainingLines(usage) <= SCORECARD_CANDIDATE_MAX_REMAINING_LINES
  || getUsageRatio(usage) >= SCORECARD_CANDIDATE_MIN_RATIO
);
...
export const buildScorecardCandidateItems = usages => usages
  .filter(isScorecardCandidate)
  .sort(compareHighUsage)
  .map(toBudgetUsageItem);
