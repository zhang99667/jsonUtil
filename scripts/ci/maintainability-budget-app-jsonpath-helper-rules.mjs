...
export const appJsonPathHelperMaintainabilityBudgets = [
  ...
  appJsonPathHelperBudget('frontend/src/utils/jsonPathPanelPreviewItems.ts', 45, 'JSONPath 面板预览项 helper 只维护可见结果裁剪、紧凑文本和预览行展示文案映射'),
  appJsonPathHelperBudget('frontend/src/utils/jsonPathPanelPreviewItems.test.ts', 75, 'JSONPath 面板预览项测试只锁定 sourceLabel、紧凑文本、展示文案和可见上限裁剪'),
  ...
  appJsonPathHelperBudget('frontend/src/hooks/useJsonPathSavedQueryLists.ts', 95, 'JSONPath 保存查询 hook 只维护收藏/历史初始化、存储同步接线和增删操作'),
  appJsonPathHelperBudget('frontend/src/hooks/useJsonPathSavedQueryLists.test.ts', 125, 'JSONPath 保存查询 hook 测试只锁定初始化、存储同步接线和列表 updater'),
  appJsonPathHelperBudget('frontend/src/hooks/useJsonPathSavedQueryListStorageSync.ts', 55, 'JSONPath 保存查询 storage sync hook 只维护历史收藏持久化、历史清理和备份导入刷新'),
  appJsonPathHelperBudget('frontend/src/hooks/useJsonPathSavedQueryListStorageSync.test.ts', 95, 'JSONPath 保存查询 storage sync 测试只锁定持久化、历史清理、备份刷新、监听清理和无 window 降级'),
