...
export const appJsonPathHelperMaintainabilityBudgets = [
  ...
  appJsonPathHelperBudget('frontend/src/hooks/useJsonPathSavedQueryListStorageSync.ts', 55, 'JSONPath 保存查询 storage sync hook 只维护历史收藏持久化和备份导入刷新'),
  appJsonPathHelperBudget('frontend/src/hooks/useJsonPathSavedQueryListStorageSync.test.ts', 95, 'JSONPath 保存查询 storage sync 测试只锁定持久化、备份导入刷新、监听清理和无 window 降级'),
