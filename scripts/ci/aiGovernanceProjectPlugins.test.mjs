...
test('两个 MCP 项目插件的 manifest 与 AI 入口版本保持一致', () => withCopy((root) => {
  ...
  fs.writeFileSync(file, fs.readFileSync(file, 'utf8').replace('plugins/jsonutils-governance-mcp@0.2.2', 'plugins/jsonutils-governance-mcp@0.2.1'));
  assert.match(collectProjectPluginFailures(root).join('\n'), /jsonutils-governance-mcp 入口版本必须精确匹配 manifest 0\.2\.2/);
}));
