...
export const appLayoutMaintainabilityBudgets = [
  ...
  layoutBudget('frontend/src/hooks/useWindowMouseDragListeners.ts', 35, '窗口鼠标拖拽监听 hook 只负责 mousemove、mouseup、窗口失焦挂载和清理，不承载拖拽计算'),
  layoutBudget('frontend/src/hooks/useWindowMouseDragListeners.test.ts', 65, '窗口鼠标拖拽监听测试只锁定未激活不挂载、失焦回收和 cleanup 移除监听'),
