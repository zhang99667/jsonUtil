...
const validationRules = [
  rule('ai-governance-assets', file => (
    hasPrefix(file, ['docs/AI-', '.agents/', 'plugins/', '.codex/', '.claude/', '.cursor/']) ||
  ...
  rule('mcp-runtime', file => hasPrefix(file, ['scripts/mcp/']) || file === '.codex/config.toml' || AI_GOVERNANCE_MCP_CONFIG_FILES.includes(file), [
    command('node --test --test-reporter=dot --test-concurrency=2 scripts/mcp/*.test.mjs', 'MCP 改动必须以有界并发覆盖真实 stdio、工具清单、资源读取和固定工具调用'),
  ]),
