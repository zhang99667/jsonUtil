...
test('MCP stdio serves handoff, decision and validation tools', async (t) => {
  ...
  const handoff = await callGovernanceMcpTool(child, readMessage, 2, 'ai_handoff_brief', { top: 1, maxFiles: 3 }, MCP_TOOL_RESPONSE_TIMEOUT_MS);
  ...
  const decisions = await callGovernanceMcpTool(child, readMessage, 3, 'ai_decision_summary', { limit: 2 }, MCP_TOOL_RESPONSE_TIMEOUT_MS);
  ...
  const validation = await callGovernanceMcpTool(child, readMessage, 4, 'ai_validation_plan', { maxFiles: 5 }, MCP_TOOL_RESPONSE_TIMEOUT_MS);
  assert.equal(validation.reportType, 'jsonutils-validation-plan');
  assert.ok(Array.isArray(validation.commands));

  const scorecard = await callGovernanceMcpTool(child, readMessage, 5, 'ai_governance_scorecard', { top: 35 }, MCP_TOOL_RESPONSE_TIMEOUT_MS);
