# AI Evolution Outcome Ledger Contract

本文是 `evals/ai-governance/feedback-inbox.jsonl`、`experiments.json`、`outcomes.jsonl` 与 `trial-receipts.jsonl` 的字段级契约。通用治理只读 `docs/AI-EVOLUTION-PLAYBOOK.md`；修改或排查 grader、ledger、outcome/receipt、checker、trace、runner 或 controller 的字段、状态或协议时才读本文。
...
- `rule-project-ai-asset-ownership@v8`、subject `2026-08-30.1` 按仓库最终状态、Git workspace/index/HEAD 与合成用户态生命周期评分：项目 marketplace 索引、完整插件树/content lock、manifest `mcpServers` 指针与 direct map/`mcp_servers` companion 官方结构、必需/引用/发现/实现/测试/eval/CI-control machine universe、原始字节/blob/type/mode 分发门禁、Git 旁路红队、required CI command 可达性、严格 CLI 参数、不发出 lifecycle mutation 的诊断、显式 apply/write-lock 及冲突/禁用/特殊路径/版本负例全部通过才可形成 deterministic outcome。分发就绪度用同一次只读聚合分开 `workspaceCandidate`、`nextCommit` 与 `clone`，前后复核 source/Git inventory 并对 drift/unavailable fail closed；该报告另作为成熟度 scorecard schema v2 的第六个维度，不自动 stage/commit。插件 lifecycle 还要求稳定 source snapshot、CLI installed/available 完整互斥闭集合、连续双 state/cache inspection、每次 mutation 前后首错即停、固定 selector 无路径穿越、每个 plugin root 恰有一个 `local` 或 manifest-version leaf 且零额外项，并把 canonical `installedPath`、即时 content lock和最终重查绑定；Codex JSON 必须经 fatal UTF-8 与唯一 authority 解析，非法项目根只进入固定脱敏占位，命令专测必须进入固定 runner；命令运行时对 exit 127/ENOENT/EACCES 输出固定无值 binary-unavailable 诊断，POSIX 在失败、超时和输出超限时强制回收独立进程组，Windows 只承诺 parent best effort，项目源码不得硬编码个人应用路径。工作树或 index 不能冒充 clone-visible，且即使 HEAD 与本机 cache 通过也不证明任意真实用户已安装、当前任务 registry、runtime isolation 或 signer trust。source-state v2 已排除双 ledger并与 HEAD 分离，可防止追加结果自我失效；ownership v8 仍须先由真实 index/HEAD 证据通过，不得用 workspace 通过代替。
...
## Feedback 与 experiment

- feedback inbox v1/v2/v3 都只接受精确紧凑、闭字段、`redacted` 的 `opened/open` 事件；event ID 与 signal ID 必须分别唯一，物理 `sequence`、直接前一事件 `previousHash` 和当前事件 SHA-256 必须一致，禁止不同事件静默覆盖同一 signal。新事件 `caseRef` 必须精确等于当前 case/subject tuple；后续 skill 升版时不改写旧行，但历史 `caseRef` 只在 event ID、event hash 和原 `caseRef` tuple 同时命中显式登记时接受。禁止用数值大小、语义版本范围、缺段/混用分隔符等版本外形，或独立 `caseVersion <= current` 推断历史；unknown、future、malformed 和非数字不可验版本全部 fail closed。预登记 registration v1 experiment 只在 experiment ID、原 `caseRef` 与完整 experiment SHA-256 同时命中时保留；未登记 experiment 和 v2 必须独立精确绑定当前 case/subject。v1 固定 project MCP 注册信号 `unknown-mcp-server/codex-task-registry/self-observed-unverified`，v2 固定 Skill 行为通道缺失信号 `behavior-evidence-channel-missing/skill-trigger-eval/repository-audit`；v3 只接受绑定当前 behavior case 的维护者纠偏 `project-maintainer-correction/project-collaboration/case-bound-redacted`，并固定 `experimentId=null`，不为了收集信号虚构 experiment。三者都声明未调用模型、未自动写 ledger、不可生成 outcome。
- feedback inbox 是 trial 前置条件失败或 experiment 设计前纠偏的排队层，不是第三种 outcome。v3 triage 会进入 open signal 计数，但不增加 experiment、planned trial 或 behavior coverage。当前 schema 不支持自动 resolve/dismiss；关闭前必须先设计显式新版本和 transition 负例。Git 基线缺失时其完整性仍为 `unknown`，本地 hash 不建立身份或可信时间。
- experiment manifest 兼容无 `contractVersion` 的 registration v1 与显式 `contractVersion=2` 的 Skill 实验。两者都要求互斥 train/validation/holdout、固定 baseline/candidate、至少 3 个 paired repetitions、唯一 trial ID，未执行时只能 `planned`、`receiptId=null`；v2 还把 validation fixture 精确绑定到项目 Skill eval 的 ID、prompt 与内容摘要。
- task/fixture/base environment 只在 experiment 全局声明，不能让 arms 偷换输入；Agent projection 只含 `input.request/context`，grader projection 独立，candidate 不得读取 grader。registration v1 保持 `blocked/new-task-required`；Skill v2 虽已有 receipt v4 component verifier，但红队证明 caller Map 与仓内 Node 不能建立受保护 trust，因此 ingestion 回退为 `unavailable/protected-assignment-trust-unavailable`，execution 仍为 `prepared/external-execution-required`。两项合计 12 planned / 0 executed，全部指标 `unavailable`，behavior coverage 不变。
- behavior case 只有同时具备完整 trace policy、`execution.status=ready` 的 paired experiment 和 `ingestion.status=ready` 的摄取器，才可标为 `fresh-task-observation-ready/actionable`。schema v1 继续用 `preparation-required` 兼容表达其它未就绪状态，但必须区分原因：组件缺失使用 `evidence-channel-components-missing`；已有 `prepared/external-execution-required` 计划且 ingestion 为 `unavailable/protected-assignment-trust-unavailable` 时投影精确 reason/stage，下一步是在仓外受保护环境执行已有 trial，而不是继续增加 component scaffold。两者都不增加 executed trial、behavior coverage 或写账权限。
- `prepare-ai-evolution-feedback.mjs` 只从固定 `mcp-server-unregistered`、`skill-behavior-channel-missing` 或 `maintainer-correction` profile 生成 candidate 到 stdout；前两者绑定固定 experiment，维护者纠偏必须显式传入已存在的 behavior `--case-id` 且固定不绑 experiment。禁止写 inbox、receipt、outcome 或长期 rule。项目插件的本机安装和 server 冒烟只算注册候选，必须在新任务的实际工具清单中完成 canary。
...
- fixed runner 只重放当前版本 case，不写 `trial-receipts.jsonl` 或 `outcomes.jsonl`。case-run/eval/MCP summary schema v3 以闭字段 `failureClass/reasonCode` 区分四类失败：普通非零按 descriptor 的 `evidenceScope` 将 `deterministic-case` 归为 `behavior-fail`、`component-only` 归为 `component-fail`；descriptor 明示的 index/HEAD 交付前置为 `delivery-blocked`，spawn、timeout、输出上限、signal、无退出状态与 binding 漂移为 `infrastructure-invalid`。禁止用自由 stderr/stdout 文本分类，聚合优先级固定 infrastructure > behavior > component > delivery；四类都失败且不可入账，只有 behavior 进入行为失败统计，component 与 infrastructure 使行为质量保持 unknown，delivery 为 warn。registration canary grader calibration 的 oracle taxonomy 独立不变。`record-ai-evolution-deterministic-outcomes.mjs` 默认 preview；只有本地维护者复核候选后才可显式 `--write`。CI、GitHub Actions、hook、postinstall 与 validation plan 均不得生成该写入参数，也不得手工构造、复制或修改 JSONL 行。
- `mcp-newline-version-negotiation` v4 / `jsonutils-governance` 0.6.0 在既有 35-case corpus 中同时锁定官方 cancellation 与 runtime freshness：每次工具调用由 fresh worker 载入当前 transitive ESM 实现，常驻 controller import closure 漂移后固定返回 restart-required；取消会抢占 worker 进程组并保持原请求零响应、typed ID 隔离、initialize 不可取消、竞态至多一响应、会话恢复和断开 abort-all。该回归只证明当前 server/component wire behavior；本轮不写 receipt/outcome，因此不自动恢复历史 behavior coverage。
...
- deterministic writer v1.1.0、unverified writer v1.2.0 与 paired writer v1.1.0 共用闭合 recovery 结果：只含 `status/transactionId/ledgerMutationPerformed/ledgerMutations`，顶层 mutation 必须等于本次 receipts/outcomes 实际追加的逻辑 OR。成功恢复只可返回 false/false、false/true 或 true/true，true/false 是不可能状态；journal cleanup 不算 ledger mutation，`recovered`/`recovered-stale` 不自动意味发生追加，`already-current` 也只是 recovery 后 candidate 状态。
- 这些保证是合作式本地串行化、崩溃恢复与仓内 tamper evidence，不是跨文件原子事务，也不证明执行者身份、可信时间、完整发布、at-most-once 或 non-equivocation。

### Unverified trace authoring

- `record-ai-evolution-unverified-trace-outcome.mjs` 从原始 stdin 只接受至多 64 KiB、fatal UTF-8 解码后的精确紧凑 JSON；非法字节不得替换为 U+FFFD 后继续解析。根对象固定为 `schemaVersion`、`artifactType=ai-evolution-unverified-trace-observation`、`dataClass=redacted`、`caseId`、`method=model|human|hybrid`、`trace`；`trace` 只含 `adapter`、`capture`、`events`。events 必须是 own index 稠密且从 1 连续递增的 record；sparse hole、敏感字段、额外字段和 caller 提供的 validation event 一律拒绝。
...
- writer v1.2.0 默认只 preview；`ready` 与 `already-current` 两条返回路径都必须二次复核 source-state v2 revision 以及双 ledger 的 endpoint/原始 bytes 未变化。`observable-trace-receipt-boundary` v3/subject 1.2.0 还锁定 recovery 补齐 trace ledger 后 `already-current` 报告保留本次真实 mutation。只有本地维护者显式 `--write` 才复用 deterministic writer 的 owner-only lock/journal、receipt-first 和精确前缀恢复事务。CI、GitHub Actions、hook、postinstall 与 validation plan 禁止该写入参数，也不得手工改 JSONL。
...
### Paired receipt v4 authoring

- `record-ai-evolution-paired-outcome.mjs` 从 stdin 只接受至多 512 KiB 的精确 `JSON.stringify` 紧凑、闭字段、`redacted` batch；这不是 RFC 8785/JCS。根对象固定绑定 experiment/case/fixture/policy/rubric/environment/source-state revision、pre-execution assignment、pre-unblind grade-set checkpoint、六条 trial 和 assignment/checkpoint/final 三份 DSSE envelope；不接受 caller verdict、score、revision、validation、chain、trust 或原始内容。
- assignment proof 在执行前绑定 batch nonce 与精确 `trial/pair/arm/treatment/blindAlias/lease/task` mapping；checkpoint 同时绑定 assignment envelope 精确字节和盲态 result/grade set；final 再绑定揭盲 batch。三角色 keyid 与 Ed25519 SPKI 必须两两不同，nonce 和三段 proof 不得换 receipt ID 重放，从而拒绝 checkpoint 后 arm relabel。
- candidate 要求唯一 `selected` evolver 决策；baseline 要求唯一 `skipped`，不得读任何 skill source，且必须按同一摘要精确读取除 evolver `SKILL.md` 外的全部共享 required reads，否则是 treatment contamination 并使整批 infrastructure-invalid。六条中任一 infrastructure-invalid 都使 reducer 返回空，不缩分母；只有三条 candidate grade 用于未来 outcome。
- ledger 除 nonce/三段 proof 外还拒绝相同 experiment/case 下的六条 `trial/lease/task/result` execution facts 换 nonce 重签，改 grade 不得伪装成新执行 receipt；这只是当前仓内 ledger 的重放可见性，不证明仓外 first-write-wins、at-most-once 或 non-equivocation。
- 仓内 Node 无法证明 caller 的 Map、环境摘要或 signer principal 受保护；三签数学通过也只是 `signature-verified-unwitnessed`，这些公钥只计 `signatureVerificationKeys`，不计 `trustedSigners`，并固定 `trustPolicyProtected=false`、`scoringEligible=false`。paired writer v1.1.0 未来的受保护授权分支会复用同一闭合 recovery mutation 投影；当前 experiment 的 sealed environment 仍 unavailable，所以 project CLI 即使 `--write` 也不 acquire/recover/写 ledger，不存在 receipt-only orphan。未来只能由 checkout 外受保护、非 caller 可控的 assignment/environment/trust authority 建立新的授权边界后才能重新开放候选写入。
...
lineage key 为 `(caseId, caseVersion, subjectVersion)`：
...
- receipt v4 的六条 trial 全部留在 receipt，但未来 outcome 只能声明三条 candidate trial；任一 infrastructure-invalid、环境未绑定、proof unknown/unwitnessed 或保护授权缺失都不可生成 paired outcome。v4 与 v1-v3 分开路由，不能放宽旧版 64 KiB、单 trace、`all-pass` 或 proof 契约。
...
- `codex-exec-jsonl@1.2.1` 只是纯 JSONL projector：生产闭包不导入 `child_process`、不启动 `codex exec`、不读取 HOME/CODEX_HOME 或登录态。原始字节 framing 对完整/跨 chunk/未终止行统一执行 1 MiB 上限，超限只计一次 drop、丢弃到下一 LF 后恢复，未终止超限行再标记 truncated；严格 UTF-8、最多 10,000 个源事件和最多 200 个 trace 事件也 fail closed。它对 `0.132.0` 与 `0.144.0-alpha.4` 的 terminal、EOF/drain、exit、item 配对、lag/drop/unknown 和隐私投影只做 component 检查；外部报告的执行事实始终标记为 unverified。
- `codex-fixed-mcp-trial@1.3.1` 的仓库 CLI 只做 host-pinned binary digest 与 0700 空 HOME/CODEX_HOME 下的 keyless `--version` preflight，携带 Codex/OpenAI key 时在导入仓库模块前拒绝。profile 只有 `executable=false` component descriptor，不包含 `codex exec` 或同 UID stdio MCP 启动计划；离线 verifier 只接受至多 2 MiB 的闭字段 JSON artifact，不接受 callback，并以空认证根执行 validation。
...
### GitHub/Sigstore detached candidate
